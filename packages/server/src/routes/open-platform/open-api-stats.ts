...
const trend = defineOpenAPIRoute({
  ...
  handler: async (c) => {
    const { startTime, endTime, granularity } = c.req.valid('query');
    return c.json(okBody(await getOpenApiStatsTrend({ startTime, endTime, granularity })), 200);
  },
});
...
const byApp = defineOpenAPIRoute({
  ...
  handler: async (c) => {
    ...
    return c.json(okBody(await getOpenApiStatsByApp({ startTime, endTime, limit })), 200);
  },
});
...
const byEndpoint = defineOpenAPIRoute({
  ...
  handler: async (c) => {
    const { startTime, endTime, limit } = c.req.valid('query');
    return c.json(okBody(await getOpenApiStatsByEndpoint({ startTime, endTime, limit })), 200);
  },
});
...
const logs = defineOpenAPIRoute({
  ...
  handler: async (c) => {
    const { page, pageSize, clientId, success, keyword, startTime, endTime } = c.req.valid('query');
    return c.json(
      okBody(
