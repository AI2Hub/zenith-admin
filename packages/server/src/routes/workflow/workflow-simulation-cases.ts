...
const saveRoute = defineOpenAPIRoute({
  route: createRoute({
    ...
    security: [{ BearerAuth: [] }],
    middleware: [authMiddleware, guard({ permission: 'workflow:definition:update', audit: { description: '保存流程仿真用例', module: '流程仿真' } })] as const,
...
const deleteRoute = defineOpenAPIRoute({
  route: createRoute({
    ...
    security: [{ BearerAuth: [] }],
    middleware: [authMiddleware, guard({ permission: 'workflow:definition:update', audit: { description: '删除流程仿真用例', module: '流程仿真' } })] as const,
