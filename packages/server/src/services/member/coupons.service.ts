...
function couponCreateToRow(input: CreateCouponInput): typeof coupons.$inferInsert {
  // 草稿允许保存不完整配置；上架（active）时必须配置完整的有效期
  const validStart = parseDateTimeInput(input.validStart ?? undefined);
  const validEnd = parseDateTimeInput(input.validEnd ?? undefined);
...
function couponUpdateToRow(input: UpdateCouponInput, existing: CouponRow): Partial<typeof coupons.$inferInsert> {
...
export const couponService = defineCrudService(couponContract, {
  table: coupons,
  map: mapCoupon,
  notFound: '优惠券不存在',
  unique: '优惠券创建失败',
  ...
  create: { toRow: couponCreateToRow },
  update: { toRow: couponUpdateToRow },
});

export const {
  list: listCoupons,
  ensure: ensureCouponExists,
  get: getCoupon,
  create: createCoupon,
  update: updateCoupon,
  remove: deleteCoupon,
