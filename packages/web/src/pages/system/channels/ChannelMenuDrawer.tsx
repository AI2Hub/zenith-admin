...
export function ChannelMenuDrawer({ channelId, channelName, visible, readOnly = false, onClose }: Readonly<Props>) {
  ...
  const menusQuery = useChannelMenus(channelId, visible && !!channelId);
  const saveMenusMutation = useSaveChannelMenus();

  useEffect(() => {
    if (visible && menusQuery.data) {
      setTops(menusQuery.data.map((t) => ({
        ...fromMenu(t),
        children: (t.children ?? []).map(fromMenu),
      })));
    }
  }, [visible, menusQuery.data]);
