...
/** 表格区高度：目录 ~2,400 行必须虚拟滚动，Semi 要求数字型 scroll.y */
function useTableHeight(offset: number) {
  const [height, setHeight] = useState(() => Math.max(240, window.innerHeight - offset));
  useEffect(() => {
    const onResize = () => setHeight(Math.max(240, window.innerHeight - offset));
...
export interface ApiCatalogFilterBarProps {
  readonly rows: readonly CatalogRow[];
  readonly filters: CatalogFilters;
  readonly onChange: (patch: Partial<CatalogFilters>) => void;
...
/** 目录筛选栏：一次取全量、客户端即时过滤，没有「查询」按钮语义 */
export function ApiCatalogFilterBar({ rows, filters, onChange, onReset, extra, primaryExtra, extraFilters }: ApiCatalogFilterBarProps) {
  ...
  return (
    <InstantFilterToolbar
      primary={(
        <>
          {primaryExtra}
          <KeywordInput placeholder="搜索名称 / 路径 / 权限码（支持拼音）" value={filters.keyword} onChange={(keyword) => onChange({ keyword })} width={280} />
        </>
      )}
      filters={(
        <>
          <FilterSelect placeholder="全部模块" items={domains} value={filters.domain} onChange={(domain) => onChange({ domain })} width={130} filter />
          <FilterSelect placeholder="全部方法" items={METHOD_OPTIONS} value={filters.method} onChange={(method) => onChange({ method })} width={110} />
          <FilterSelect placeholder="全部认证方式" items={SECURITY_OPTIONS} value={filters.security} onChange={(security) => onChange({ security })} width={130} />
          <FilterSelect placeholder="全部访问要求" items={ACCESS_KIND_OPTIONS} value={filters.accessKind} onChange={(accessKind) => onChange({ accessKind })} width={130} />
          <FilterSelect placeholder="全部权限码" items={permissions} value={filters.permission} onChange={(permission) => onChange({ permission })} width={260} filter />
          <FilterSelect placeholder="全部审计" items={AUDIT_OPTIONS} value={filters.audit} onChange={(audit) => onChange({ audit })} width={110} />
          {features.length > 0 && (
            <FilterSelect placeholder="全部功能门控" items={features} value={filters.feature} onChange={(feature) => onChange({ feature })} width={140} filter />
          )}
          {extraFilters}
        </>
      )}
...
export interface ApiCatalogTableProps {
  ...
  /** 表格之上占用的高度（工具栏 / 统计等），用于推导虚拟滚动区高度 */
  readonly heightOffset?: number;
}

export function ApiCatalogTable({ rows, onOpen, verdictOf, heightOffset = 330 }: ApiCatalogTableProps) {
  const tableHeight = useTableHeight(heightOffset);
