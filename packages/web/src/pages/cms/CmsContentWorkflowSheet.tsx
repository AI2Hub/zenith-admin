  ...
  return (
    <WorkflowSideSheet title={`内容审批：${title}`} visible onCancel={onClose} variant="split">
      <BusinessWorkflowPanel
        ...
        loading={detailQuery.isLoading || contextQuery.isLoading || previewQuery.isLoading}
        error={detailQuery.error ?? contextQuery.error ?? (contextQuery.data?.instance ? null : previewQuery.error)}
        onRetry={() => { void detailQuery.refetch(); void contextQuery.refetch(); if (!contextQuery.data?.instance) void previewQuery.refetch(); }}
