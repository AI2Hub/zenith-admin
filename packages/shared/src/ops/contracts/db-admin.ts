...
export const dbAdminContract = defineContract('/api/db-admin', {
  tables: op.get('/tables', { access: { permission: 'system:db-admin:view' }, response: z.array(dbAdminTableItemSchema), summary: '表列表' }),
  overview: op.get('/overview', { access: { permission: 'system:db-admin:view' }, response: dbAdminOverviewSchema, summary: '数据库总览' }),
  tableStructure: op.get('/tables/{schema}/{name}/structure', { access: { permission: 'system:db-admin:view' }, params: dbAdminTableParam, response: dbAdminTableStructureSchema, summary: '表结构' }),
  tableRows: op.get('/tables/{schema}/{name}/rows', { access: { permission: 'system:db-admin:view' }, params: dbAdminTableParam, query: dbAdminTableRowsQuery, response: dbAdminTableRowsSchema, summary: '表数据' }),
  insertRow: op.post('/tables/{schema}/{name}/rows', { access: { permission: 'system:db-admin:write' }, audit: '插入表数据行', params: dbAdminTableParam, body: dbAdminInsertRowSchema, response: dbAdminRowSchema, summary: '插入行' }),
  updateRow: op.patch('/tables/{schema}/{name}/rows', { access: { permission: 'system:db-admin:write' }, audit: '更新表数据行', params: dbAdminTableParam, body: dbAdminUpdateRowSchema, response: dbAdminRowSchema, summary: '更新行' }),
  deleteRow: op.delete('/tables/{schema}/{name}/rows', { access: { permission: 'system:db-admin:write' }, audit: '删除表数据行', params: dbAdminTableParam, body: dbAdminDeleteRowSchema, summary: '删除行' }),
  batchMutate: op.post('/tables/{schema}/{name}/batch-mutate', { access: { permission: 'system:db-admin:write' }, audit: '批量变更表数据行', params: dbAdminTableParam, body: dbAdminBatchMutateSchema, response: dbAdminBatchMutateResultSchema, summary: '批量变更行（事务：插入 / 更新 / 删除）' }),
  importRows: op.post('/tables/{schema}/{name}/import', { access: { permission: 'system:db-admin:write' }, audit: { description: '批量导入表数据', recordBody: false }, params: dbAdminTableParam, body: dbAdminImportRowsSchema, response: dbAdminImportResultSchema, summary: '批量导入数据（CSV / JSON）' }),
  truncateTable: op.post('/tables/{schema}/{name}/truncate', { access: { permission: 'system:db-admin:write' }, audit: '截断表 TRUNCATE', params: dbAdminTableParam, summary: '截断表（TRUNCATE）' }),
  exportTableSql: op.get('/tables/{schema}/{name}/export.sql', { access: { permission: 'system:db-admin:export' }, audit: '导出表 SQL', params: dbAdminTableParam, query: dbAdminExportSqlQuery, kind: 'file', summary: '导出表 SQL（DDL / INSERT / 完整）' }),
  exportTableCsv: op.get('/tables/{schema}/{name}/export.csv', { access: { permission: 'system:db-admin:export' }, audit: '导出表数据 CSV', params: dbAdminTableParam, kind: 'csv', summary: '导出表数据 CSV' }),
  runMaintenance: op.post('/tables/{schema}/{name}/maintenance', { access: { permission: 'system:db-admin:maintain' }, audit: '执行表维护', params: dbAdminTableParam, body: dbAdminMaintenanceActionSchema, summary: '执行表维护（VACUUM / ANALYZE / REINDEX）' }),
  refreshMatview: op.post('/tables/{schema}/{name}/refresh', { access: { permission: 'system:db-admin:maintain' }, audit: '刷新物化视图', params: dbAdminTableParam, summary: '刷新物化视图' }),
  query: op.post('/query', { access: { permission: 'system:db-admin:query' }, audit: '执行 SQL 查询', body: dbAdminQueryBodySchema, response: dbAdminQueryResultSchema, summary: '执行只读 SQL' }),
  cancelQuery: op.post('/query/cancel', { access: { permission: 'system:db-admin:query' }, audit: '取消正在执行的 SQL 查询', body: dbAdminCancelQuerySchema, response: dbAdminOpResultSchema, summary: '取消正在执行的查询' }),
  exportQueryCsv: op.post('/query/export.csv', { access: { permission: 'system:db-admin:export' }, audit: '导出 SQL 结果 CSV', body: dbAdminExportQueryBodySchema, kind: 'csv', summary: '导出 SQL 结果 CSV' }),
  exportQueryJson: op.post('/query/export.json', { access: { permission: 'system:db-admin:export' }, audit: '导出 SQL 结果 JSON', body: dbAdminExportQueryBodySchema, kind: 'file', summary: '导出 SQL 结果 JSON' }),
  explain: op.post('/explain', { access: { permission: 'system:db-admin:query' }, audit: 'EXPLAIN SQL', body: dbAdminExplainBodySchema, response: dbAdminExplainResultSchema, summary: 'EXPLAIN 查询计划' }),
  history: op.get('/query/history', { access: { permission: 'system:db-admin:view' }, query: paginationQuery, response: paginated(dbAdminQueryHistoryItemSchema), summary: '查询历史' }),
  removeHistory: op.delete('/query/history/{id}', { access: { permission: 'system:db-admin:view' }, audit: '删除查询历史', params: idParam, summary: '删除一条查询历史' }),
  clearHistory: op.delete('/query/history', { access: { permission: 'system:db-admin:view' }, audit: '清空查询历史', summary: '清空查询历史' }),
  erDiagram: op.get('/er-diagram', { access: { permission: 'system:db-admin:view' }, response: z.array(dbAdminErDiagramFkSchema), summary: 'ER 图（所有外键关系）' }),
  erSchema: op.get('/er-schema', { access: { permission: 'system:db-admin:view' }, response: dbAdminErSchemaSchema, summary: 'ER 图完整模式（表 + 列 + 外键）' }),
  favorites: op.get('/query-favorites', { access: { permission: 'system:db-admin:view' }, response: z.array(dbQueryFavoriteSchema), summary: '获取 SQL 收藏夹列表' }),
  createFavorite: op.post('/query-favorites', { access: { permission: 'system:db-admin:view' }, audit: '新增 SQL 收藏', body: createDbQueryFavoriteSchema, response: dbQueryFavoriteSchema, summary: '新增 SQL 收藏' }),
  updateFavorite: op.put('/query-favorites/{id}', { access: { permission: 'system:db-admin:view' }, audit: '更新 SQL 收藏', params: idParam, body: updateDbQueryFavoriteSchema, response: dbQueryFavoriteSchema, summary: '更新 SQL 收藏' }),
  removeFavorite: op.delete('/query-favorites/{id}', { access: { permission: 'system:db-admin:view' }, audit: '删除 SQL 收藏', params: idParam, summary: '删除 SQL 收藏' }),
  activity: op.get('/activity', { access: { permission: 'system:db-admin:view' }, response: z.array(dbAdminActivityConnectionSchema), summary: '活动连接列表' }),
  cancelBackend: op.post('/activity/{pid}/cancel', { access: { permission: 'system:db-admin:maintain' }, audit: '取消数据库查询', params: dbAdminBackendPidParam, response: dbAdminOpResultSchema, summary: '取消查询' }),
  terminateBackend: op.post('/activity/{pid}/terminate', { access: { permission: 'system:db-admin:maintain' }, audit: '终止数据库连接', params: dbAdminBackendPidParam, response: dbAdminOpResultSchema, summary: '终止连接' }),
  maintenanceTables: op.get('/maintenance/tables', { access: { permission: 'system:db-admin:view' }, response: z.array(dbAdminTableMaintenanceSchema), summary: '表维护统计' }),
  indexHealth: op.get('/index-health', { access: { permission: 'system:db-admin:view' }, response: dbAdminIndexHealthSchema, summary: '索引健康' }),
  objects: op.get('/objects', { access: { permission: 'system:db-admin:view' }, response: dbAdminObjectsSchema, summary: '数据库对象（序列 / 函数 / 触发器 / 枚举 / 扩展）' }),
