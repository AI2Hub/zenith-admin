...
export const workflowDefinitionContract = defineContract('/api/workflows/definitions', {
  ...
  update: op.put('/{id}', { access: { permission: 'workflow:definition:edit' }, audit: '更新流程定义', params: idParam, body: updateWorkflowDefinitionSchema, response: workflowDefinitionSchema, summary: '更新流程定义' }),
  publish: op.post('/{id}/publish', { access: { permission: 'workflow:definition:publish' }, audit: '发布流程定义', params: idParam, response: workflowDefinitionSchema, summary: '发布流程' }),
  disable: op.post('/{id}/disable', { access: { permission: 'workflow:definition:publish' }, audit: '禁用流程定义', params: idParam, response: workflowDefinitionSchema, summary: '禁用流程' }),
  enable: op.post('/{id}/enable', { access: { permission: 'workflow:definition:publish' }, audit: '启用流程定义', params: idParam, response: workflowDefinitionSchema, summary: '启用流程' }),
  remove: op.delete('/{id}', { access: { permission: 'workflow:definition:delete' }, audit: '删除流程定义', params: idParam, summary: '删除流程' }),
  batchDisable: op.post('/batch-disable', { access: { permission: 'workflow:definition:publish' }, audit: '批量禁用流程定义', body: batchIdsBody, summary: '批量禁用流程' }),
  batchEnable: op.post('/batch-enable', { access: { permission: 'workflow:definition:publish' }, audit: '批量启用流程定义', body: batchIdsBody, summary: '批量启用流程' }),
  batchDelete: op.post('/batch-delete', { access: { permission: 'workflow:definition:delete' }, audit: '批量删除流程定义', body: batchIdsBody, summary: '批量删除流程' }),
  versions: op.get('/{id}/versions', { access: { permission: 'workflow:definition:list' }, params: idParam, query: paginationQuery, response: paginated(workflowDefinitionVersionSchema), summary: '历史版本列表' }),
  restoreVersion: op.post('/{id}/versions/{versionId}/restore', { access: { permission: 'workflow:definition:edit' }, audit: '恢复历史版本', params: workflowDefinitionVersionParam, response: workflowDefinitionSchema, summary: '恢复历史版本' }),
  duplicate: op.post('/{id}/duplicate', { access: { permission: 'workflow:definition:create' }, audit: '复制流程', params: idParam, response: workflowDefinitionSchema, summary: '复制流程' }),
  export: op.get('/{id}/export', { access: { permission: 'workflow:definition:list' }, params: idParam, response: workflowDefinitionExportSchema, summary: '导出流程定义' }),
  diff: op.get('/{id}/diff', { access: { permission: 'workflow:definition:list' }, params: idParam, query: workflowVersionDiffQuery, response: workflowVersionDiffSchema, summary: '版本对比' }),
  preview: op.post('/{id}/preview', { access: { permission: 'workflow:instance:create' }, params: idParam, body: previewWorkflowSchema, response: z.array(workflowApproverPreviewNodeSchema), summary: '提交前审批链路预览' }),
  simulate: op.post('/simulate', { access: { permission: 'workflow:definition:list' }, body: simulateWorkflowSchema, response: workflowSimulationResultSchema, summary: '流程仿真' }),
  healthCheck: op.post('/health-check', { access: { permission: 'workflow:definition:list' }, body: workflowHealthCheckSchema, response: workflowDefinitionHealthReportSchema, summary: '发布前健康体检（评分+分支覆盖）' }),
}, { auditModule: '工作流管理', tags: ['WorkflowDefinitions'] });
