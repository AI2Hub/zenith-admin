...
export const workflowInstanceOpsContract = defineContract('/api/workflows', {
  diagnostics: op.get('/instances/{id}/diagnostics', { access: { permission: 'workflow:instance:monitor' }, params: idParam, response: workflowRuntimeDiagnosticsSchema, summary: '实例运行时技术诊断' }),
  trace: op.get('/instances/{id}/trace', { access: { permission: 'workflow:instance:monitor' }, params: idParam, response: workflowInstanceTraceSchema, summary: '实例运行轨迹与引擎解释' }),
  tokens: op.get('/instances/{id}/tokens', { access: { permission: 'workflow:instance:monitor' }, params: idParam, response: workflowExecutionTokenViewSchema, summary: '实例显式执行 Token（执行树/活动路径）' }),
  diagnosticBundle: op.get('/instances/{id}/diagnostic-bundle', { access: { permission: 'workflow:instance:monitor' }, params: idParam, response: workflowDiagnosticBundleSchema, summary: '导出实例诊断包' }),
  batchSkipStuck: op.post('/instances/batch-skip-stuck', { access: { permission: 'workflow:instance:monitor' }, audit: '批量推进卡死实例', body: batchSkipStuckTokensSchema, response: workflowRecoveryBatchResultSchema, summary: '批量推进卡在指定节点的实例' }),
  skipToken: op.post('/instances/tokens/{id}/skip', { access: { permission: 'workflow:instance:monitor' }, audit: { description: '跳过卡死执行 Token', recordResponseBody: false }, params: idParam, body: workflowTokenOpSchema, response: workflowInstanceSchema, summary: '跳过卡死的执行 Token' }),
  replayToken: op.post('/instances/tokens/{id}/replay', { access: { permission: 'workflow:instance:cancel' }, audit: { description: '从执行 Token 重放流程', recordResponseBody: false }, params: idParam, body: workflowTokenOpSchema, response: workflowInstanceSchema, summary: '从执行 Token 节点重放流程' }),
  jump: op.post('/instances/{id}/jump', { access: { permission: 'workflow:instance:cancel' }, audit: { description: '强制跳转流程节点', recordResponseBody: false }, params: idParam, body: jumpWorkflowInstanceSchema, response: workflowInstanceSchema, summary: '管理员强制跳转节点' }),
  suspend: op.post('/instances/{id}/suspend', { access: { permission: 'workflow:engine:operate' }, audit: { description: '挂起流程实例', recordResponseBody: false }, params: idParam, body: suspendWorkflowInstanceSchema, response: workflowInstanceSchema, summary: '挂起流程实例' }),
  resume: op.post('/instances/{id}/resume', { access: { permission: 'workflow:engine:operate' }, audit: { description: '恢复挂起流程实例', recordResponseBody: false }, params: idParam, response: workflowInstanceSchema, summary: '恢复挂起的流程实例' }),
  migratePreflight: op.get('/{id}/migrate/preflight', { access: { permission: 'workflow:engine:operate' }, params: idParam, response: workflowMigrationPreflightSchema, summary: '实例迁移预检' }),
  migrate: op.post('/{id}/migrate', { access: { permission: 'workflow:engine:operate' }, audit: '迁移流程实例', params: idParam, summary: '迁移实例到最新版本' }),
  migrations: op.get('/{id}/migrations', { access: { permission: 'workflow:instance:monitor' }, params: idParam, response: z.array(workflowInstanceMigrationSchema), summary: '实例迁移记录' }),
  migrateBatch: op.post('/migrate/batch/{definitionId}', { access: { permission: 'workflow:engine:operate' }, audit: '批量迁移流程实例', params: workflowMigrateBatchParam, summary: '批量迁移定义下运行实例' }),
  compensations: op.get('/compensation/list', { access: { permission: 'workflow:instance:monitor' }, query: workflowCompensationListQuery, response: paginated(workflowCompensationSchema), summary: '补偿/修复工单列表' }),
  resolveCompensation: op.post('/compensation/{id}/resolve', { access: { permission: 'workflow:engine:operate' }, audit: '处理补偿工单', params: idParam, body: resolveWorkflowCompensationSchema, response: workflowCompensationSchema, summary: '处理补偿工单' }),
  addCompensationNote: op.post('/compensation/{id}/note', { access: { permission: 'workflow:engine:operate' }, audit: '补偿工单添加备注', params: idParam, body: addWorkflowCompensationNoteSchema, response: workflowCompensationDetailSchema, summary: '补偿工单：添加处理备注/附件' }),
  retryCompensation: op.post('/compensation/{id}/retry', { access: { permission: 'workflow:engine:operate' }, audit: '重试补偿动作', params: idParam, response: workflowCompensationDetailSchema, summary: '补偿工单：重试自动反向动作' }),
  resumeCompensation: op.post('/compensation/{id}/resume', { access: { permission: 'workflow:engine:operate' }, audit: '恢复流程推进', params: idParam, response: workflowCompensationDetailSchema, summary: '补偿工单：恢复后继续推进' }),
  compensationDetail: op.get('/compensation/{id}', { access: { permission: 'workflow:instance:monitor' }, params: idParam, response: workflowCompensationDetailSchema, summary: '补偿工单详情（含处理历史）' }),
