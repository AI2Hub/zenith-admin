...
export const SEED_CRON_JOBS: CronJob[] = [
  ...
  },
  {
    ...
    name: '工作流子流程恢复',
    cronExpression: '*/5 * * * *',
    handler: 'recoverStuckWorkflowSubProcesses',
    params: null,
    status: 'enabled',
  ...
  {
    id: 18,
    name: '工作流事件 Outbox 重放',
    cronExpression: '* * * * *',
    handler: 'replayWorkflowEventOutbox',
    params: null,
    status: 'enabled',
    description: '每分钟重放工作流事件 outbox 中未处理或失败的事件，保证订阅、触发器、外部审批等运行时副作用可恢复',
    retryCount: 0,
    retryInterval: 0,
    retryBackoff: false,
    monitorTimeout: null,
