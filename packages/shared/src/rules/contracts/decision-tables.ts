...
export const decisionTableContract = defineContract('/api/rules/decision-tables', {
  list: op.get('/', { access: { permission: 'rule:table:list' }, query: decisionTableListQuery, response: paginated(ruleDecisionTableSchema), summary: '决策表分页列表' }),
  detail: op.get('/{id}', { access: { permission: 'rule:table:list' }, params: idParam, response: ruleDecisionTableSchema, summary: '决策表详情' }),
  versions: op.get('/{id}/versions', { access: { permission: 'rule:table:list' }, params: idParam, response: z.array(ruleDecisionTableVersionSchema), summary: '决策表版本列表' }),
  diff: op.get('/{id}/diff', { access: { permission: 'rule:table:list' }, params: idParam, query: decisionTableDiffQuery, response: ruleVersionDiffSchema, summary: '版本对比（0=当前编辑态）' }),
  rollback: op.post('/{id}/rollback/{version}', { access: { permission: 'rule:table:update' }, audit: '回滚决策表版本', params: ruleVersionParam, response: ruleDecisionTableSchema, summary: '回滚到历史版本' }),
  usages: op.get('/{id}/usages', { access: { permission: 'rule:table:list' }, params: idParam, response: z.array(ruleUsageItemSchema), summary: '决策表引用分析（where-used）' }),
  stats: op.get('/{id}/stats', { access: { permission: 'rule:table:list' }, params: idParam, query: decisionTableStatsQuery, response: ruleTableStatsSchema, summary: '命中分析（近 N 天执行流水聚合）' }),
  shadowRun: op.post('/{id}/shadow-run', { access: { permission: 'rule:table:evaluate' }, params: idParam, body: shadowRunDecisionTableSchema, response: ruleShadowRunResultSchema, summary: '影子对比（重放最近执行输入到编辑态）' }),
  submitReview: op.post('/{id}/submit-review', { access: { permission: 'rule:table:publish' }, audit: '申请发布决策表', params: idParam, response: ruleDecisionTableSchema, summary: '申请发布（审批模式，先过发布门禁）' }),
  review: op.post('/{id}/review', { access: { permission: 'rule:table:approve' }, audit: '审批决策表发布', params: idParam, body: reviewDecisionTableSchema, response: ruleDecisionTableSchema, summary: '审批发布（四眼：批准执行发布 / 驳回记录意见）' }),
  cases: op.get('/{id}/cases', { access: { permission: 'rule:table:list' }, params: idParam, response: z.array(ruleTestCaseSchema), summary: '测试用例列表' }),
  createCase: op.post('/{id}/cases', { access: { permission: 'rule:table:update' }, audit: '新增决策表用例', params: idParam, body: createRuleTestCaseSchema, response: ruleTestCaseSchema, summary: '新增测试用例' }),
  runCases: op.post('/{id}/cases/run', { access: { permission: 'rule:table:evaluate' }, params: idParam, response: ruleTestRunResultSchema, summary: '批量运行用例（覆盖率）' }),
  updateCase: op.put('/{id}/cases/{caseId}', { access: { permission: 'rule:table:update' }, audit: '更新决策表用例', params: ruleTestCaseParam, body: updateRuleTestCaseSchema, response: ruleTestCaseSchema, summary: '更新测试用例' }),
  removeCase: op.delete('/{id}/cases/{caseId}', { access: { permission: 'rule:table:update' }, audit: '删除决策表用例', params: ruleTestCaseParam, summary: '删除测试用例' }),
  create: op.post('/', { access: { permission: 'rule:table:create' }, audit: '创建决策表', body: createDecisionTableSchema, response: ruleDecisionTableSchema, summary: '创建决策表' }),
  update: op.put('/{id}', { access: { permission: 'rule:table:update' }, audit: '更新决策表', params: idParam, body: updateDecisionTableSchema, response: ruleDecisionTableSchema, summary: '更新决策表' }),
  publish: op.post('/{id}/publish', { access: { permission: 'rule:table:publish' }, audit: '发布决策表', params: idParam, body: publishDecisionTableSchema, response: ruleDecisionTableSchema, summary: '发布决策表（可选灰度：新版本按主体分桶生效，其余流量走上一版本）' }),
  grayAction: op.post('/{id}/gray', { access: { permission: 'rule:table:publish' }, audit: '决策表灰度操作', params: idParam, body: grayActionSchema, response: ruleDecisionTableSchema, summary: '灰度操作：complete=转正全量；cancel=放弃（旧版本前滚为新版本）' }),
  simulate: op.post('/{id}/simulate', { access: { permission: 'rule:table:evaluate' }, params: idParam, body: simulateDecisionTableSchema, response: ruleSimulateResultSchema, summary: '批量仿真（逐行以编辑态求值，汇总命中率与规则行分布）' }),
  toggle: op.post('/{id}/toggle', { access: { permission: 'rule:table:publish' }, audit: '启用/停用决策表', params: idParam, body: toggleDecisionTableSchema, response: ruleDecisionTableSchema, summary: '启用/停用决策表' }),
  test: op.post('/{id}/test', { access: { permission: 'rule:table:evaluate' }, params: idParam, body: evaluateDecisionTableSchema, response: ruleEvaluateResultSchema, summary: '测试求值（编辑态）' }),
  evaluate: op.post('/evaluate', { access: { permission: 'rule:table:evaluate' }, body: evaluateRuleByKeySchema, response: ruleEvaluateResultSchema, summary: '按 key 求值（对外通用，支持 zat_ API Token 调用）' }),
  removeBatch: op.delete('/batch', { access: { permission: 'rule:table:delete' }, audit: '批量删除决策表', body: batchIdsBody, summary: '批量删除决策表' }),
  remove: op.delete('/{id}', { access: { permission: 'rule:table:delete' }, audit: '删除决策表', params: idParam, summary: '删除决策表' }),
}, { auditModule: '规则中心', tags: ['DecisionTables'] });
