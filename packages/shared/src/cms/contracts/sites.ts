...
export const cmsSiteContract = defineContract('/api/cms/sites', {
  ...
  themes: op.get('/themes', { access: { permission: 'cms:site:list' }, query: cmsThemeScopeQuery, response: z.array(cmsThemeSchema), summary: '可用主题列表' }),
  themeTemplates: op.get('/themes/{code}/templates', { access: { permission: 'cms:site:list' }, params: cmsThemeCodeParam, query: cmsThemeScopeQuery, response: cmsThemeTemplateManifestSchema, summary: '主题可选模板清单（站点默认模板/栏目/内容模板下拉）' }),
  themeSettingsSchema: op.get('/themes/{code}/settings-schema', { access: { permission: 'cms:site:list' }, params: cmsThemeCodeParam, response: z.array(cmsThemeSettingFieldSchema), summary: '主题参数声明（后台主题参数面板动态表单）' }),
  templateHealth: op.get('/{id}/template-health', { access: { permission: 'cms:site:list' }, params: idParam, query: cmsTemplateHealthQuery, response: cmsTemplateHealthSchema, summary: '站点模板健康检查（扫描站点/栏目/内容的失效模板引用；?theme= 预检切换目标主题）' }),
  inheritanceChain: op.get('/{id}/inheritance-chain', { access: { permission: 'cms:site:list' }, params: idParam, response: z.array(cmsSiteChainNodeSchema), summary: '查看站点继承链（隐藏无权父级）' }),
  effectiveConfig: op.get('/{id}/effective-config', { access: { permission: 'cms:site:list' }, params: idParam, response: cmsSiteEffectiveConfigSchema, summary: '有效配置及逐项来源（secret 始终掩码）' }),
  move: op.put('/{id}/parent', { access: { permission: 'cms:site:hierarchy' }, audit: '移动 CMS 站点子树', params: idParam, body: moveCmsSiteSchema, response: cmsSiteMoveResultSchema, summary: '安全移动站点子树' }),
  updateInheritance: op.put('/{id}/inheritance', { access: { permission: 'cms:site:hierarchy' }, audit: '更新 CMS 站点继承策略', params: idParam, body: updateCmsSiteInheritanceSchema, response: cmsSiteInheritanceUpdateResultSchema, summary: '逐项覆盖或恢复继承' }),
  detail: op.get('/{id}', { access: { permission: 'cms:site:list' }, params: idParam, response: cmsSiteSchema, summary: '站点详情' }),
  create: op.post('/', { access: { permission: 'cms:site:create' }, audit: '创建 CMS 站点', body: createCmsSiteSchema, response: cmsSiteSchema, summary: '创建站点' }),
  update: op.put('/{id}', { access: { permission: 'cms:site:update' }, audit: '更新 CMS 站点', params: idParam, body: updateCmsSiteSchema, response: cmsSiteSchema, summary: '更新站点' }),
  remove: op.delete('/{id}', { access: { permission: 'cms:site:delete' }, audit: '删除 CMS 站点', params: idParam, summary: '删除站点' }),
  users: op.get('/{id}/users', { access: { permission: 'cms:site:list' }, params: idParam, response: cmsAuthorizedUsersSchema, summary: '站点授权用户' }),
  setUsers: op.put('/{id}/users', { access: { permission: 'cms:site:update' }, audit: '设置 CMS 站点授权用户', params: idParam, body: setCmsAuthorizedUsersSchema, summary: '设置站点授权用户（绑定后仅授权用户可管理该站点）' }),
  enableAnalytics: op.post('/{id}/enable-analytics', { access: { permission: 'cms:site:update' }, audit: 'CMS 站点开通行为统计', params: idParam, response: cmsSiteAnalyticsEnableResultSchema, summary: '开通行为统计（自动创建统计站点，前台注入采集脚本）' }),
  import: op.post('/import', { access: { permission: 'cms:site:create' }, audit: '导入 CMS 站点', body: cmsSiteImportPackageSchema, response: cmsSiteImportResultSchema, summary: '导入站点（上传导出包 JSON，创建为新站点）' }),
  export: op.get('/{id}/export', { access: { permission: 'cms:site:update' }, audit: '导出 CMS 站点', params: idParam, kind: 'file', summary: '导出站点（结构 + 内容整站打包为 JSON 附件，不含运行数据）' }),
  openGrants: op.get('/{id}/open-grants', { access: { permission: 'cms:site:update' }, params: idParam, response: z.array(cmsOpenAppGrantSchema), summary: '站点的开放应用授权列表' }),
  saveOpenGrant: op.put('/{id}/open-grants', { access: { permission: 'cms:site:update' }, audit: '设置 CMS 站点开放授权', params: idParam, body: saveCmsOpenAppGrantSchema, response: cmsOpenAppGrantSchema, summary: '授权开放应用写入本站点（未授权一律拒绝）' }),
  removeOpenGrant: op.delete('/open-grants/{grantId}', { access: { permission: 'cms:site:update' }, audit: '删除 CMS 站点开放授权', params: cmsOpenGrantIdParam, summary: '删除开放应用授权' }),
}, { auditModule: 'CMS内容管理', tags: ['CMS-站点管理'] });
