...
export const iotDeviceContract = defineContract('/api/iot/devices', {
  list: op.get('/', { access: { permission: 'iot:device:list' }, query: iotDeviceListQuery, response: paginated(iotDeviceSchema), summary: '设备列表（含在线态与最近指标快照）' }),
  removeBatch: op.delete('/batch', { access: { permission: 'iot:device:delete' }, audit: '批量删除 IoT 设备', body: batchIdsBody, summary: '批量删除设备' }),
  detail: op.get('/{id}', { access: { permission: 'iot:device:list' }, params: idParam, response: iotDeviceSchema, summary: '设备详情（含接入凭证与实时状态）' }),
  telemetryAgg: op.get('/{id}/telemetry/agg', {
    access: { permission: 'iot:telemetry:view' },
    params: idParam,
  ...
  listCommands: op.get('/{id}/commands', { access: { permission: 'iot:command:send' }, params: idParam, query: paginationQuery, response: paginated(iotCommandSchema), summary: '指令下发记录' }),
  sendCommand: op.post('/{id}/commands', {
    access: { permission: 'iot:command:send' }, audit: '下发 IoT 指令',
    params: idParam,
    body: sendIotCommandSchema,
    response: iotCommandSchema,
    summary: '下发指令（WS 在线即时推送，离线等待上线补推）',
  }),
  resetSecret: op.post('/{id}/reset-secret', { access: { permission: 'iot:device:update' }, audit: '重置 IoT 设备密钥', params: idParam, response: iotDeviceSchema, summary: '重置接入密钥（旧密钥立即失效）' }),
  clearTelemetry: op.delete('/{id}/telemetry', { access: { permission: 'iot:device:update' }, audit: '清空 IoT 设备遥测', params: idParam, summary: '清空设备遥测数据' }),
  shadow: op.get('/{id}/shadow', { access: { permission: 'iot:device:list' }, params: idParam, response: iotDeviceShadowSchema, summary: '设备影子（reported / desired / 在线标记）' }),
  setDesired: op.put('/{id}/shadow/desired', {
    access: { permission: 'iot:command:send' }, audit: '设置 IoT 期望属性',
    params: idParam,
    body: setIotDesiredSchema,
    response: iotDeviceShadowSchema,
    summary: '设置期望属性（rw 属性，按物模型校验；WS 在线即时推送）',
  }),
  clearDesired: op.delete('/{id}/shadow/desired', { access: { permission: 'iot:command:send' }, audit: '清空 IoT 期望属性', params: idParam, response: iotDeviceShadowSchema, summary: '清空期望属性（放弃未确认的下发）' }),
  events: op.get('/{id}/events', {
    access: { permission: 'iot:device:list' },
    params: idParam,
    query: iotDeviceEventListQuery,
    response: paginated(iotDeviceEventSchema),
    summary: '设备事件流（生命周期 + 物模型事件，倒序）',
  }),
  topology: op.get('/{id}/topology', { access: { permission: 'iot:device:list' }, params: idParam, response: iotDeviceTopologySchema, summary: '网关拓扑（子设备 + 在线态 + 活跃告警数）' }),
  logs: op.get('/{id}/logs', {
    access: { permission: 'iot:device:list' },
    params: idParam,
