...
export const driveNodeContract = defineContract('/api/drive/nodes', {
  ensureDirectories: op.post('/folders/ensure', {
    access: { permission: 'drive:node:upload' },
    body: ensureDriveDirectoriesSchema,
    response: z.array(z.object({ path: z.string(), nodeId: z.int() })),
    summary: '创建或复用上传目录树',
  }),
  list: op.get('/', { access: { permission: 'drive:node:list' }, query: driveNodeListQuery, response: driveNodeListResultSchema, summary: '目录内容（parentId 缺省 = 空间根级）' }),
  search: op.get('/search', { access: { permission: 'drive:node:list' }, query: driveNodeSearchQuery, response: paginated(driveSearchItemSchema), summary: '搜索（名称，可选正文全文）' }),
  sharedWithMe: op.get('/shared-with-me', { access: { permission: 'drive:node:list' }, query: driveNodeViewQuery, response: paginated(driveSharedItemSchema), summary: '与我共享' }),
  starred: op.get('/starred', { access: { permission: 'drive:node:list' }, query: driveNodeViewQuery, response: paginated(driveStarredItemSchema), summary: '我的收藏' }),
  recent: op.get('/recent', { access: { permission: 'drive:node:list' }, query: driveNodeViewQuery, response: paginated(driveRecentItemSchema), summary: '最近访问' }),
  recycle: op.get('/recycle', { access: { permission: 'drive:recycle:list' }, query: driveRecycleQuery, response: paginated(driveRecycleItemSchema), summary: '回收站' }),
  restore: op.post('/recycle/restore', { access: { permission: 'drive:recycle:restore' }, audit: '还原网盘文件', body: driveNodeIdsSchema, summary: '从回收站还原' }),
  purge: op.post('/recycle/purge', { access: { permission: 'drive:recycle:purge' }, audit: '彻底删除网盘文件', body: driveNodeIdsSchema, summary: '彻底删除' }),
  emptyRecycle: op.delete('/recycle', { access: { permission: 'drive:recycle:purge' }, audit: '清空网盘回收站', query: driveEmptyRecycleQuery, summary: '清空回收站（可按空间）' }),
  createFolder: op.post('/folder', { access: { permission: 'drive:node:edit' }, audit: '新建网盘文件夹', body: createDriveFolderSchema, response: driveNodeSchema, summary: '新建文件夹' }),
  move: op.post('/move', { access: { permission: 'drive:node:edit' }, audit: '移动网盘文件', body: moveDriveNodesSchema, summary: '移动到目标目录（同空间）' }),
  copy: op.post('/copy', { access: { permission: 'drive:node:edit' }, audit: '复制网盘文件', body: copyDriveNodesSchema, response: driveCopyResultSchema, summary: '复制到目标目录（大目录树转任务中心）' }),
  removeBatch: op.delete('/batch', { access: { permission: 'drive:node:delete' }, audit: '删除网盘文件', body: driveNodeIdsSchema, summary: '删除到回收站' }),
  batchDownload: op.post('/batch-download', {
    access: { permission: 'drive:node:download' }, audit: '打包下载网盘文件',
  ...
  precheck: op.post('/precheck', { access: { permission: 'drive:node:upload' }, body: driveUploadPrecheckSchema, response: driveUploadPrecheckResultSchema, summary: '上传预检（冲突 / 配额 / 秒传）' }),
  upload: op.post('/upload', { access: { permission: 'drive:node:upload' }, audit: { description: '上传网盘文件', recordBody: false }, body: driveUploadBody, response: driveNodeSchema, summary: '简单上传（单请求；超过分片阈值 files.chunkThresholdMb 需改用分片）' }),
  uploadInit: op.post('/upload/init', { access: { permission: 'drive:node:upload' }, audit: '初始化网盘分片上传', body: driveUploadInitSchema, response: uploadSessionInitSchema, summary: '初始化分片上传' }),
  uploadChunk: op.post('/upload/chunk', { access: { permission: 'drive:node:upload' }, body: driveUploadChunkBody, response: uploadChunkResultSchema, summary: '上传单个分片' }),
  uploadComplete: op.post('/upload/complete', { access: { permission: 'drive:node:upload' }, audit: '完成网盘分片上传', body: driveUploadCompleteSchema, response: driveNodeSchema, summary: '完成分片上传并落地为节点' }),
  uploadStatus: op.get('/upload/{uploadId}/status', { access: { permission: 'drive:node:upload' }, params: driveUploadIdParam, response: uploadSessionStatusSchema, summary: '分片上传进度' }),
  uploadAbort: op.delete('/upload/{uploadId}', { access: { permission: 'drive:node:upload' }, params: driveUploadIdParam, summary: '中止分片上传' }),
  detail: op.get('/{id}', { access: { permission: 'drive:node:list' }, params: idParam, response: driveNodeDetailSchema, summary: '节点详情' }),
  rename: op.put('/{id}/rename', { access: { permission: 'drive:node:edit' }, audit: '重命名网盘文件', params: idParam, body: renameDriveNodeSchema, response: driveNodeSchema, summary: '重命名' }),
  content: op.get('/{id}/content', { access: { permission: 'drive:node:list' }, params: idParam, query: driveNodeContentQuery, kind: 'file', summary: '文件内容（预览需 viewer；?download=1 需 downloader）' }),
  thumbnail: op.get('/{id}/thumbnail', { access: { permission: 'drive:node:list' }, params: idParam, kind: 'file', summary: '缩略图' }),
  accessUrl: op.get('/{id}/access-url', { access: { permission: 'drive:node:download' }, params: idParam, query: driveNodeAccessUrlQuery, response: fileAccessUrlSchema, summary: '解析访问直链（presigned / public；proxy 回落到鉴权地址）' }),
  versions: op.get('/{id}/versions', { access: { permission: 'drive:node:list' }, params: idParam, response: z.array(driveFileVersionSchema), summary: '版本列表' }),
  uploadVersion: op.post('/{id}/versions', { access: { permission: 'drive:node:upload' }, audit: { description: '上传网盘文件新版本', recordBody: false }, params: idParam, body: driveUploadVersionBody, response: driveNodeSchema, summary: '上传新版本（单请求；超过分片阈值用分片 init 传 nodeId）' }),
  versionContent: op.get('/{id}/versions/{version}/content', { access: { permission: 'drive:node:download' }, params: driveNodeVersionParams, kind: 'file', summary: '历史版本内容' }),
  restoreVersion: op.post('/{id}/versions/{version}/restore', { access: { permission: 'drive:node:edit' }, audit: '回滚网盘文件版本', params: driveNodeVersionParams, response: driveNodeSchema, summary: '回滚到历史版本（生成新版本）' }),
  removeVersion: op.delete('/{id}/versions/{version}', { access: { permission: 'drive:node:delete' }, audit: '删除网盘文件历史版本', params: driveNodeVersionParams, summary: '删除历史版本' }),
  permissions: op.get('/{id}/permissions', { access: { permission: 'drive:node:list' }, params: idParam, response: driveNodePermissionsResultSchema, summary: '节点授权（直接 + 继承）' }),
  savePermissions: op.put('/{id}/permissions', { access: { permission: 'drive:node:grant' }, audit: '保存网盘节点授权', params: idParam, body: saveDriveNodePermissionsSchema, response: driveNodePermissionsResultSchema, summary: '全量保存节点直接授权（需 manager）' }),
  setInherit: op.put('/{id}/inherit', { access: { permission: 'drive:node:grant' }, audit: '变更网盘节点继承', params: idParam, body: setDriveNodeInheritSchema, response: driveNodePermissionsResultSchema, summary: '断开 / 恢复继承（需 manager）' }),
  activities: op.get('/{id}/activities', { access: { permission: 'drive:node:list' }, params: idParam, query: paginationQuery, response: paginated(driveActivitySchema), summary: '节点动态' }),
  comments: op.get('/{id}/comments', { access: { permission: 'drive:node:list' }, params: idParam, response: z.array(driveNodeCommentSchema), summary: '评论列表' }),
  createComment: op.post('/{id}/comments', { access: { permission: 'drive:node:list' }, params: idParam, body: createDriveNodeCommentSchema, response: driveNodeCommentSchema, summary: '发表评论' }),
  removeComment: op.delete('/{id}/comments/{commentId}', { access: { permission: 'drive:node:list' }, params: driveNodeCommentParams, summary: '删除评论（作者或 manager）' }),
  star: op.post('/{id}/star', { access: { permission: 'drive:node:list' }, params: idParam, summary: '收藏' }),
  unstar: op.delete('/{id}/star', { access: { permission: 'drive:node:list' }, params: idParam, summary: '取消收藏' }),
  setTags: op.put('/{id}/tags', { access: { permission: 'drive:node:edit' }, params: idParam, body: setDriveNodeTagsSchema, response: driveNodeSchema, summary: '设置节点标签' }),
  lock: op.post('/{id}/lock', { access: { permission: 'drive:node:edit' }, audit: '锁定网盘文件', params: idParam, body: lockDriveNodeSchema, response: driveNodeSchema, summary: '签出锁定' }),
  unlock: op.delete('/{id}/lock', { access: { permission: 'drive:node:edit' }, audit: '解锁网盘文件', params: idParam, response: driveNodeSchema, summary: '解除锁定' }),
  shareLinks: op.get('/{id}/share-links', { access: { permission: 'drive:node:list' }, params: idParam, response: z.array(driveShareLinkSchema), summary: '节点外链（manager 见全部，其他人见自己创建的）' }),
  createShareLink: op.post('/{id}/share-links', { access: { permission: 'drive:link:create' }, audit: { description: '创建网盘外链', recordBody: false }, params: idParam, body: createDriveShareLinkSchema, response: driveShareLinkSchema, summary: '创建外链（需 editor + drive:link:create）' }),
  presence: op.get('/{id}/presence', { access: { permission: 'drive:node:list' }, params: idParam, response: z.array(drivePresenceUserSchema), summary: '正在查看该节点的用户（心跳 60 秒内）' }),
  heartbeat: op.post('/{id}/presence', { access: { permission: 'drive:node:list' }, params: idParam, response: z.array(drivePresenceUserSchema), summary: '上报我正在查看该节点，并返回当前在线用户' }),
  leavePresence: op.delete('/{id}/presence', { access: { permission: 'drive:node:list' }, params: idParam, summary: '离开该节点（立即从在线列表移除）' }),
  sendToChat: op.post('/{id}/send-to-chat', { access: { permission: 'drive:node:list' }, params: idParam, body: sendDriveNodeToChatSchema, response: z.object({ sent: z.int() }), summary: '以卡片消息把文件 / 文件夹链接发送到聊天会话（需 viewer；收件人仍按网盘 ACL 访问）' }),
}, { auditModule: '企业网盘', tags: ['企业网盘-文件'] });
