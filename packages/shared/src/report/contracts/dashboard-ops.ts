...
export const reportDashboardOpsContract = defineContract('/api/report/dashboards', {
  versions: op.get('/{id}/versions', { access: { permission: 'report:dashboard:list' }, params: idParam, response: z.array(reportDashboardVersionSchema), summary: '版本列表' }),
  createVersion: op.post('/{id}/versions', { access: { permission: 'report:dashboard:update' }, audit: '保存仪表盘版本', params: idParam, body: createReportVersionSchema, response: reportDashboardVersionSchema, summary: '保存版本快照' }),
  versionDiff: op.get('/{id}/versions/diff', { access: { permission: 'report:dashboard:list' }, params: idParam, query: reportVersionDiffQuerySchema, response: reportDashboardVersionDiffSchema, summary: '版本差异比较' }),
  restoreVersion: op.post('/{id}/versions/{versionId}/restore', { access: { permission: 'report:dashboard:update' }, audit: '恢复仪表盘版本', params: reportVersionRestoreParam, body: restoreReportVersionSchema, summary: '恢复版本' }),
  favorite: op.post('/{id}/favorite', { access: { permission: 'report:dashboard:list' }, params: idParam, response: reportFavoriteResultSchema, summary: '收藏 / 取消收藏' }),
  shares: op.get('/{id}/shares', { access: { permission: 'report:dashboard:update' }, params: idParam, response: z.array(reportDashboardShareSchema), summary: '分享链接列表' }),
  createShare: op.post('/{id}/shares', { access: { permission: 'report:dashboard:update' }, audit: { description: '创建报表分享链接', recordResponseBody: false }, params: idParam, body: createReportShareSchema, response: reportDashboardShareSchema, summary: '创建分享链接' }),
  updateShare: op.put('/shares/{shareId}', { access: { permission: 'report:dashboard:update' }, audit: { description: '更新报表分享链接', recordResponseBody: false }, params: reportShareIdParam, body: updateReportShareSchema, response: reportDashboardShareSchema, summary: '更新分享链接' }),
  removeShare: op.delete('/shares/{shareId}', { access: { permission: 'report:dashboard:update' }, audit: '删除报表分享链接', params: reportShareIdParam, summary: '删除分享链接' }),
  embedTokens: op.get('/{id}/embed-tokens', { access: { permission: 'report:dashboard:update' }, params: idParam, response: z.array(reportDashboardEmbedTokenSchema), summary: '嵌入令牌列表' }),
  createEmbedToken: op.post('/{id}/embed-tokens', { access: { permission: 'report:dashboard:update' }, audit: { description: '创建仪表盘嵌入令牌', recordResponseBody: false }, params: idParam, body: createReportEmbedTokenSchema, response: reportDashboardEmbedTokenSchema, summary: '创建嵌入令牌' }),
  revokeEmbedToken: op.post('/embed-tokens/{embedTokenId}/revoke', { access: { permission: 'report:dashboard:update' }, audit: '撤销仪表盘嵌入令牌', params: reportEmbedTokenIdParam, summary: '撤销嵌入令牌' }),
  comments: op.get('/{id}/comments', { access: { permission: 'report:dashboard:list' }, params: idParam, query: reportCommentListQuerySchema, response: paginated(reportDashboardCommentSchema), summary: '评论列表' }),
  createComment: op.post('/{id}/comments', { access: { permission: 'report:dashboard:list' }, params: idParam, body: createReportCommentSchema, response: reportDashboardCommentSchema, summary: '发表评论' }),
  updateComment: op.put('/{id}/comments/{commentId}', { access: { permission: 'report:dashboard:list' }, params: reportCommentIdParam, body: updateReportCommentSchema, response: reportDashboardCommentSchema, summary: '编辑评论' }),
  resolveComment: op.post('/{id}/comments/{commentId}/resolve', { access: { permission: 'report:dashboard:list' }, params: reportCommentIdParam, body: resolveReportCommentSchema, response: reportDashboardCommentSchema, summary: '解决 / 重新打开评论' }),
  removeComment: op.delete('/{id}/comments/{commentId}', { access: { permission: 'report:dashboard:list' }, params: reportCommentIdParam, summary: '删除评论' }),
}, { auditModule: '报表仪表盘', tags: ['报表仪表盘'] });
