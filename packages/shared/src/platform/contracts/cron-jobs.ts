...
import { auditFieldsSchema, dateRangeQuery, entityStatusQuery, idParam, idQuery, keywordQuery, paginated, paginationQuery, queryEnum } from '../../core/api-schemas';
...
export const cronJobContract = defineContract('/api/cron-jobs', {
  handlers: op.get('/handlers', { access: { permission: 'system:cronjob:list' }, response: z.array(z.string()), summary: '已注册 Handler' }),
  validate: op.post('/validate', { access: { permission: 'system:cronjob:list' }, body: cronValidateSchema, response: cronValidateResultSchema, summary: '校验 Cron 表达式' }),
  list: op.get('/', { access: { permission: 'system:cronjob:list' }, query: cronJobListQuery, response: paginated(cronJobSchema), summary: '任务列表' }),
  logs: op.get('/logs', { access: { permission: 'system:cronjob:list' }, query: cronJobLogListQuery, response: paginated(cronJobLogSchema), summary: '所有执行日志' }),
  clearLogs: op.delete('/logs/clean', { access: { permission: 'system:cronjob:delete' }, audit: '清除所有执行日志', query: cronJobClearLogsQuery, summary: '清除所有执行日志' }),
  stats: op.get('/stats', { access: { permission: 'system:cronjob:list' }, query: cronJobStatsQuery, response: cronJobStatsSchema, summary: '任务统计' }),
  create: op.post('/', { access: { permission: 'system:cronjob:create' }, audit: '新增任务', body: createCronJobSchema, response: cronJobSchema, summary: '新增任务' }),
  detail: op.get('/{id}', { access: { permission: 'system:cronjob:list' }, params: idParam, response: cronJobSchema, summary: '任务详情' }),
  update: op.put('/{id}', { access: { permission: 'system:cronjob:update' }, audit: '更新任务', params: idParam, body: updateCronJobSchema, response: cronJobSchema, summary: '更新任务' }),
  remove: op.delete('/{id}', { access: { permission: 'system:cronjob:delete' }, audit: '删除任务', params: idParam, summary: '删除任务' }),
  run: op.post('/{id}/run', { access: { permission: 'system:cronjob:execute' }, audit: '手动执行任务', params: idParam, summary: '手动执行' }),
  setStatus: op.put('/{id}/status', { access: { permission: 'system:cronjob:update' }, audit: '切换任务状态', params: idParam, body: cronJobStatusSchema, summary: '切换状态' }),
  jobLogs: op.get('/{id}/logs', { access: { permission: 'system:cronjob:list' }, params: idParam, query: paginationQuery, response: paginated(cronJobLogSchema), summary: '单任务日志' }),
  jobStats: op.get('/{id}/stats', { access: { permission: 'system:cronjob:list' }, params: idParam, query: cronJobStatsQuery, response: cronJobDetailStatsSchema, summary: '单任务执行统计' }),
  clearJobLogs: op.delete('/{id}/logs/clean', { access: { permission: 'system:cronjob:delete' }, audit: '清除单任务执行日志', params: idParam, query: cronJobClearLogsQuery, summary: '清除单任务执行日志' }),
}, { auditModule: '定时任务', tags: ['CronJobs'] });
