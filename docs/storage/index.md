...
个人签名模板使用 restricted 托管文件，由身份域本人接口读取；模板更换/删除通过引用计数释放旧文件；删除账号或租户同样在事务内释放模板引用，租户删除时仅将这些受控文件记录转为无租户的待回收记录，避免级联删除元数据后无法清理对象；已签单据持有独立图片快照，见[我的签名](../iam/profile.md)。
