...
- 上传成功后写入 `managed_files`，返回统一访问 URL：`/api/files/{id}/content`。
...
- 当前实现采用本地临时目录暂存 + 流式合并（对全部 provider 通用）；`upload_sessions.multipart_upload_id` 与 `upload_chunks.etag` 为后续接入各云原生 multipart 预留。
- 前端工具位于 `packages/web/src/utils/chunked-upload.ts`，`uploadId` 持久化到 `localStorage`，刷新或重新选择同一文件可续传。
- **临时分片**暂存于 `packages/server/storage/tmp/uploads/{uploadId}/{index}`。完成（`complete`）或中止（`abort`）时立即删除整个会话目录；对**中断未完成**的会话，由定时任务 `cleanupUploadSessions`（默认每天 4:30）按 `upload_session_ttl_hours`（默认 24 小时）清理过期会话、临时分片与孤儿目录（孤儿目录额外做 mtime 校验，避免误删进行中的上传）。
...
| `GET` | `/api/file-storage-configs/export` | `system:file:config` | 导出 Excel |
| `GET` | `/api/file-storage-configs/export/csv` | `system:file:config` | 导出 CSV |
...
| `business_id` | 业务记录 ID |
| `file_id` | 关联 `managed_files.id`，删除文件时级联删除 |
| `name` | 业务侧展示名称 |
| `category` | 业务分类 |
| `sort_order` | 排序值 |
| `tenant_id` | 租户 ID |
