...
| 访问直链 | 按存储配置 `urlStrategy` 三级策略解析访问地址：`proxy`（服务端代理，兜底）/ `public`（永久公开直链）/ `presigned`（临时签名直链），卸载代理流量 |
...
| `url_strategy` / `urlStrategy` | 访问策略：`proxy`（默认）/ `public` / `presigned` |
| `public_base_url` / `publicBaseUrl` | 自定义访问域名（CDN/加速域名），public 策略优先使用 |
| `presigned_expiry_seconds` / `presignedExpirySeconds` | 临时签名有效期（秒），默认 1800，限 60~604800 |
| `remark` | 备注 |

### 访问直链策略（URL Strategy）

URL 不入库，读取时按存储配置动态解析，降级链 `public → presigned → proxy`：

- **proxy**（默认）：返回 `/api/files/{id}/content` 服务端代理路径，流量经过应用服务器，适合 local/sftp 及兜底场景。
- **public**：拼接永久公开直链（`publicBaseUrl` 优先，否则按 provider 默认公网域名），浏览器直连对象存储。有对象级 ACL 的云（oss/s3/cos/obs/bos）要求文件上传时的 ACL 快照为 `public-read*`，否则自动降级 presigned；无对象级 ACL 的 provider（kodo/azure/local/sftp）信任管理员配置。public 直链会直接进入列表接口的 `url` 字段（永久有效，可安全缓存）。
- **presigned**：用各云官方 SDK 本地签发限时下载直链（纯本地 HMAC 计算，不发网络请求），适合私有文件。local/sftp 不支持，自动降级 proxy。

上传时会在 `managed_files.object_acl` 快照实际发送的对象 ACL（`null` = 继承 Bucket，公开性未知，保守不走公开直链）；分片上传在初始化时快照到 `upload_sessions` 并在完成时拷贝。

**按需获取直链**：`GET /api/files/{id}/access-url?purpose=preview|download`（需登录）返回 `{ url, strategy, expiresAt }`，presigned 每次签发新鲜 URL，响应带 `Cache-Control: private, no-store`，前端禁止长期缓存；`purpose=download` 时 oss/s3/azure 直链附带 attachment disposition。前端 `fetchManagedFileBlob()` 会自动先换直链再下载，失败（CORS/过期）时降级回代理路径。

> 使用 public/presigned 直链做浏览器内预览（blob fetch）时，需在对象存储侧配置 CORS 允许站点来源；仅 `<a>` 下载/新窗口打开无需 CORS。
...
存储配置支持设置**上传对象的读写权限**（canned ACL），后续上传的文件将自动带上该权限；`default`（继承 Bucket）表示不单独指定、跟随 Bucket 权限（上传时不发送 ACL 参数）。简单上传与分片上传（multipart init）均生效。

各 provider 支持矩阵（由 `@zenith/shared` 的 `FILE_OBJECT_ACL_SUPPORT` 统一定义，前端表单选项、Zod 校验、上传链路三处共用）：

| Provider | default | private | public-read | public-read-write | 实现方式 |
| --- | --- | --- | --- | --- | --- |
| `oss` | ✓ | ✓ | ✓ | ✓ | `x-oss-object-acl` 请求头 |
| `s3` | ✓ | ✓ | ✓ | ✓ | `ACL` 参数 |
| `cos` | ✓ | ✓ | ✓ | — | `ACL` 参数（对象级不支持公共读写） |
| `obs` | ✓ | ✓ | ✓ | ✓ | `ACL` 参数 |
| `bos` | ✓ | ✓ | ✓ | — | `x-bce-acl` 请求头（对象级不支持公共读写） |
| `local` / `kodo` / `azure` / `sftp` | — | — | — | — | 不支持对象级 ACL |

注意事项：

- **S3**：AWS 新建桶默认启用「Bucket owner enforced」（禁用 ACL），带 ACL 上传会报 `AccessControlListNotSupported`，需先在桶设置中开启 ACL；MinIO / Cloudflare R2 不支持对象 ACL，请保持 `default`。
- **公共读 / 公共读写**存在数据泄露风险，前端表单已作提示，请谨慎选择。
- 该权限只影响对象在云端的直接访问；管理后台内的预览 / 下载统一经服务端代理（AK 鉴权），不受此设置影响。
- 更新接口为 partial schema，service 层会按支持矩阵对非法组合**兜底回落为 `default`**（如切换 provider 后残留不支持的取值）。

---

## 上传安全

上传逻辑位于 `packages/server/src/services/files/`。
...
文件配置页菜单路径为 `/system/file-configs`。
...
统一预览组件为 `FilePreviewModal`，详细能力见[文件预览组件](../frontend/file-preview.md)。
