...
- **固定路由**：硬编码在 `App.tsx` 中，始终可访问，例如 `/login`、`/profile`、`/notifications`
...
运行时会在 `packages/web/src/App.tsx` 中按 `./pages/${m.component}.tsx` 的形式拼接并懒加载；例如上面的 `system/users/UsersPage` 最终对应 `packages/web/src/pages/system/users/UsersPage.tsx`。
