...
| 表单能力 | 表单库、自定义业务表单、业务系统主导表单、远程数据源 | 支持可视化表单、React 自定义页面、`bizType + bizId` 外部业务关联和远程选项拉取 |
...
| 集成能力 | 事件订阅、节点监听器、外部审批、连接器 | 通过 HMAC 签名、连接器鉴权、重试、熔断对接外部系统 |
| 自动化与定时 | 流程自动化、定时发起 | 在实例创建/结束时执行联动动作，或按 Cron 由指定发起人自动发起流程 |
| 监控运维 | 流程监控、数据分析、健康巡检、引擎诊断、作业账本、补偿工单 | 观测实例、任务、执行 Token、异步作业、事件投递、子流程、效率瓶颈和健康趋势，并执行恢复动作 |
...
| `draft` | 用户保存但尚未提交的申请 |
| `running` | 正在流转 |
| `suspended` | 管理员挂起，待办冻结、计时暂停，恢复后续跑 |
| `approved` | 所有路径完成并通过 |
| `rejected` | 节点驳回并按策略终止 |
| `withdrawn` | 发起人撤回 |
| `cancelled` | 管理员强制取消 |
