...
流程设计器是配置工作流的核心工具，采用类钉钉/飞书的垂直流式布局，支持点击添加节点、配置节点属性、设计分支条件等操作。
...
| 可发起范围 | 否 | 限制哪些用户、部门或角色可以发起此流程，详见 [权限与范围控制](./permissions.md) |

> 已发布的流程被修改后，状态会自动回退到草稿，需要重新发布才能生效。

### 第二步：表单设计

配置用户在发起申请时需要填写的表单。设计器可从「表单库」选择一个已有表单进行绑定复用，并提供表单预览。详见 [表单设计](./form-design.md)。
...
| 预览 | 在表单步骤点击「预览」按钮，查看当前绑定表单的渲染效果 |
...
| 设置项 | 说明 |
| --- | --- |
| 允许撤回 | 是否允许发起人在运行中撤回流程 |
| 允许驳回后重新提交 | 流程被驳回后是否允许发起人修改并重新提交 |
| 允许流程中评论 | 是否允许在流程处理过程中发表评论 |
| 流程结束后通知发起人 | 流程通过/驳回后是否给发起人发送站内信通知 |
| 相同审批人自动通过 | 当某节点审批人与已审批人相同时是否自动通过（去重审批的全局开关） |
| 超时处理 | 流程级超时兜底动作：不处理 / 自动同意 / 自动驳回 / 仅通知 |
| 业务编号 | 按前缀、日期格式、流水位数和重置周期生成实例业务编号 |
| 通知渠道 | 配置邮件、短信等流程级通知渠道 |
...
**历史版本**：发布后可在「流程定义」列表或设计器画布中点击「历史版本」，查看历史快照、进行版本对比，并恢复为草稿后重新编辑发布。
...
4. **测试验证**：发布前使用「预览」功能验证表单渲染是否符合预期
