...
实例详情采用普通流程统一的两栏布局：左侧业务资料与协作页签，右侧审批链、当前处理状态，流程图从辅助入口展开。业务系统主导流程的业务表单入口也复用这套详情；提交前则展示审批链路预览，接入规范见 [业务模块接入工作流](./business-integration.md#5-在业务表单内展示流程)。

| 页签 | 说明 |
| --- | --- |
| 表单 | 渲染发起时表单或业务查看组件 |
| 流转记录 | 任务记录，可选择审批环节、抄送或全部留痕范围 |
| 沟通 | 评论与提及 |
| 协办 | 协办请求和回复 |
| 子流程 | 父子实例关系与跳转 |

业务表单上的「审批轮次」可在当前审批/本次提审预览与过去实例之间切换；过去流程使用所选实例的定义快照和任务记录，业务表单仍展示当前业务资料，不作为历史业务快照。
