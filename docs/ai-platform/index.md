...
前端页面位于 `packages/web/src/pages/ai/chat/AIChatPage.tsx`，菜单路径为 `/ai/chat`。页面采用左右主从布局：
...
当前流式适配器位于 `packages/server/src/lib/ai/adapters/openai-compatible.ts`。调用层统一按 OpenAI Compatible 的 `/chat/completions` 协议发送请求，支持 `stream: true` 的 SSE 增量响应。
...
管理员页面位于 `packages/web/src/pages/ai/feedback/AiFeedbackPage.tsx`，菜单路径为 `/ai/feedback`。页面支持按反馈类型与处理状态筛选，并可将反馈状态更新为「待处理」「已处理」或「已忽略」，同时填写处理备注。

---

## 接口一览

所有 AI 接口均在 `packages/server/src/index.ts` 下挂载到 `/api/ai/*`。`ai-chat` 与 `ai-conversations` 都挂载在 `/api/ai/conversations`，其中聊天接口为该路径下的流式子路由。
...
AI 模块表定义在 `packages/server/src/db/schema.ts`。
...
| 页面 | 路由 | 组件 | 说明 |
| --- | --- | --- | --- |
| 智能对话 | `/ai/chat` | `packages/web/src/pages/ai/chat/AIChatPage.tsx` | 多轮对话、模型选择、提示词角色、PDF 预览、反馈、会话管理 |
| AI 服务商 | `/ai/providers` | `packages/web/src/pages/ai/providers/AIProvidersPage.tsx` | 系统级服务商配置管理、启停、默认切换、连接测试 |
| AI 反馈 | `/ai/feedback` | `packages/web/src/pages/ai/feedback/AiFeedbackPage.tsx` | 反馈筛选与处理 |
| 提示词模板 | `/ai/prompts` | `packages/web/src/pages/ai/prompts/PromptTemplatesPage.tsx` | 模板 CRUD、范围筛选、启停展示 |
| 用量统计 | `/ai/usage` | `packages/web/src/pages/ai/usage/AiUsagePage.tsx` | 概览卡片、趋势图、模型统计、用户 Top 10 |
| 我的 AI 配置 | 智能对话页侧边弹窗 | `packages/web/src/pages/ai/components/UserAiConfigModal.tsx` | 个人配置查看、创建、编辑、删除 |
