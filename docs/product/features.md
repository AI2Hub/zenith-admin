...
| 文件管理器 | 服务器文件系统浏览、上传/下载、重命名、删除、权限查看与 **chmod 修改**、**压缩(ZIP) / 解压(zip/tar/gz/tgz/bz2/xz)**、**递归深度搜索**、**文件校验和(MD5/SHA1/SHA256)** |
...
| 模块 | 当前能力 |
| --- | --- |
| 支付渠道 | 支付渠道（微信支付/支付宝等）的创建、编辑、启用/禁用与默认切换，详见 [Payment](../backend/payment/index.md) |
...
| 模块 | 当前能力 |
| --- | --- |
| 会员管理 | 会员 CRUD、状态管理、重置密码、登录方式与来源展示 |
| 会员等级 | 等级体系维护（成长值阈值 / 折扣 / 权益配置）、与会员账号动态关联 |
| 积分管理 | 积分账户与流水记录、管理员手动调整（乘法锁防并发） |
| 钉包管理 | 钉包余额与永久流水、调整余额、退款处理（金额单位为分、乘法锁） |
| 优惠券 | 优惠券模板（满减券/折扣券）CRUD、发券记录与使用状态查询 |
| 前台 C 端 | 独立 SPA（`member.html`），支持手机号验证码 / 密码登录、会员资料编辑、积分/钉包/卡券浏览 |
