...
| 模块 | 当前能力 |
| --- | --- |
| 文件管理 | 文件上传、列表、预览/下载、删除、批量下载、网格/列表双视图 |
...
| 定时任务 | 任务 CRUD、Cron 配置、启停与手动执行、执行记录，详见[定时任务](../backend/cron-jobs.md) |
| 数据库备份 | 手动创建/下载/删除备份，详见[数据库与迁移](../backend/database.md#数据库备份) |
| 数据库管理 | SQL 控制台（Monaco Editor、只读事务）、表数据浏览与编辑、ER 图、批量操作 |
...
| 触发器执行 | updateData / deleteData 触发器类型、fallbackStrategy 兜底策略 |
| 外部审批 | 外部审批配置与回调地址查看、触发器执行记录 |
...
| 用户偏好 | 文件视图模式（列表/网格）、表格列显隐配置、侧边栏粘性效果、标签页动画等 |
| 标签页管理 | 多标签右键菜单（刷新/关闭/关闭其他/关闭全部）、拖拽排序、最大数量限制 |
