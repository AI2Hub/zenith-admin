...
- 数据管理页继续保留「采集设置」专用页面，并从系统设置新增「数据采集」专用入口与跳转。
- 采集配置统一接入 `system_settings` 的租户级 `analytics` 模块，保留平台继承、版本并发控制、权限和缓存失效能力。
- SDK 公开采集配置、错误忽略、IP 匿名化、会话回放配额与数据保留策略统一读取系统设置。

### Removed

- 删除旧 `analytics_settings` 表及 `/api/analytics/settings` 专用读写接口。
...
- 新增「系统监控 → SQL 监控」页面，支持查询统计、活动会话、锁与阻塞、历史采样、取消 / 终止会话和 SQL 统计重置。
- SQL 采样纳入固定系统调度任务，原始采样接入数据保留策略，默认保留 14 天。

### Changed

- 重建数据库迁移基线，将当前 schema 收敛到 `0000_baseline.sql`，并将扩展、分区和其他手写 DDL 收敛到 `0001_extensions.sql`。
- 仓库自带 PostgreSQL Compose 服务启动时预加载 `pg_stat_statements`；外部数据库继续按部署配置启用。

### Fixed

- `pg_stat_statements` 不可用时，SQL 监控明确展示降级状态，不影响其他数据库监控能力。
- SQL 查询表格的 Query ID 加宽并使用统一单行省略提示，Top SQL「查看全部」可定位到查询统计区域。
...
- 应用版本页统一支持客户端应用与服务端应用；服务端版本使用 `server` / `archive` 制品。
- 系统运维新增「应用部署」页，覆盖部署目标、异步部署记录、逐主机实时日志、滚动 / 并行发布、健康检查自动回滚、取消 / 重试、发布备份与 release 对账。
- 部署执行复用统一 SSH / SFTP 主机执行层与任务中心，支持制品上传、SHA256 校验、解包、`shared` 持久化目录、`current` 原子切换、systemd / 脚本重启和健康检查。
- 部署完成、失败、取消与回滚结果接入通知中心和 WebSocket；应用版本与应用部署页面支持版本 / 部署记录深链跳转。

### Changed

- 应用版本入口归入「系统设置 → 系统运维」，与「应用部署」形成版本、目标、运行记录和发布备份的统一运维工作区。
- 部署目录采用不可变 `releases/<timestamp-version>` + `current` 原子切换 + `shared` 持久化布局；部署目标互斥、历史 release 保护和主机目录对账由服务端统一执行。
- 数据库迁移按当前完整 schema 重建为 `0000_baseline_v238_deploy.sql` + `0001_extensions.sql`。本版本不提供旧迁移历史兼容，已有环境需按全新数据库方案重新初始化后再部署。

### Fixed

- 取消部署仅在 current 切换前中断，避免主机停在已切换但未重启的中间状态。
- 自动回滚和发布清理始终保护当前版本与上一版本，保证失败恢复路径可用。
...
新增登录会话并发控制与个人签名，业务表单内统一展示审批流程，桌面端和移动审批支持按策略签署；同时优化登录反馈与接口目录性能。

### 升级注意

- 执行 `npm run db:migrate` 应用两条迁移：`0021_login_event_kicked` 为登录日志增加被挤下线事件；`0022_personal_signatures` 新增个人签名模板表 `user_signatures` 和任务签署证据列 `workflow_tasks.signature_evidence`。
- 工作流签名采用新契约：节点使用 `signaturePolicy`，替代 `operations` 中的 `signature`；任务与待办分别返回 `signaturePolicy` / `pendingSignaturePolicy`。审批提交使用结构化 `SignatureInput`，表单签名保存为服务端生成的 `SignatureSnapshot`，不再接受旧的纯字符串签名输入。
- 自定义接入需同步：登录结果增加 `sessionConflict` 分支；接口目录移除重复的 `access` 字段，改用现有 `accessKind`、`permissions`、`platformOnly`。业务审批资料接口须携带所属 `instanceId`。

### Added

- **会话并发控制**：身份安全设置支持不限、仅一处或最多 N 处登录，可按全部终端合计或网页 / 移动审批 / 桌面端分别计算；超限可下线旧设备，或拒绝新登录并展示占用设备，确认后继续原登录及 MFA 流程。
- **设备与下线提示**：在线用户、我的设备展示终端类型和当前策略；被挤下线时提示新登录设备信息，登录日志记录 `kicked` 事件。
- **个人签名**：个人中心新增「我的签名」，支持手写保存、更换和删除，按账号与有效租户隔离，桌面端和移动审批共用。签名图片私有保存，签署时明确确认；模板更新、删除不影响已保存的单据签名。
- **业务表单中的流程信息**：请假与 CMS 工作流审核表单支持提交前预览、提交后原地查看审批链、流程图、流转记录和往次轮次；查看往次流程时明确提示业务区展示当前资料。
- **签名策略与批量签署**：节点支持无需签名、允许复用个人签名、每次手写；表单签名字段独立配置策略。符合条件的任务可一次确认后批量签署，逐条校验并返回成功或失败原因；必须手写、自选下一审批人或补充必填附件的任务仍需单独处理。
- 新增两类签名演示表单和流程模板，同步请假、CMS、Demo 与接入文档。

### Changed

- 登录、注册、MFA、目录登录和找回密码改为行内校验与错误展示；失焦检查必填项，已触碰字段随输入重新校验。
- 接口目录的查询、重置和统计筛选使用已缓存目录，不再重复下载；表格、分页和筛选项减少无效重渲染。服务端复用序列化与 gzip 结果，支持 ETag / 304 协商，认证与权限检查仍逐次执行。
- 会话查询与按用户强退使用用户会话索引，减少全量扫描；刷新令牌和切换租户视角按会话迁移处理，保留终端与登录时间。
- 移动审批拆分 QueryClient 与业务 hooks，审批图标合并为延迟加载包，减少首屏依赖和零碎请求。

### Fixed

- 修复业务审批参与者查看 CMS 资料受后台内容权限阻断的问题；按具体实例验证业务关联，重新提交后仍可查看原审批轮次。
- 请假管理日期列加宽，日期范围保持单行显示。
- 防止草稿保存、提交和审批字段更新并发覆盖已保存的签名；任务改派后，原处理人的陈旧请求不能继续落签。
- 修复批量请求重复任务 ID 被重复处理并覆盖成功结果的问题；部分失败保留逐条原因和重试入口。
- 签名图片不再进入审计请求、响应或前后差异快照，审计保留签署元信息；模拟登录不能读取、维护或使用个人签名。
...
全部能力**不依赖任何外部服务**——不做 Office / PDF 正文抽取、服务端渲染、在线编辑、AI 问答、病毒扫描与 DLP，相关适配器与依赖已移除。
新增 8 个迁移（`0004`–`0011`；其中 `0006` 的分区 DDL 为手写 SQL，其余由 `drizzle-kit generate` 生成）。

### Added

#### 阶段 0：基础重构

- ACL 生效链物化（`acl_chain_ids` / `acl_open`）：目录、搜索、收藏、最近、与我共享统一在 SQL 层精确过滤可见性，断开继承在任意层级生效
- 托管文件引用计数与延迟 GC：版本 / 渲染产物在事务内维护 `ref_count`，归零打孤儿标记，24 小时宽限后回收；`deleting` 对象不可再被秒传引用
- 持久化渲染队列 `drive-renditions`（缩略图 / 文本索引）：版本检查、重试、启动与周期补投
- 文件动态与外链访问日志按 UTC 月分区，分区预建任务与整分区保留清理
- 外链能力位模型（`kind` + `capabilities`），Range / ETag 内容代理与一次计数下载
- 独立文档目录 `docs/drive/`，首页与导航接入

#### 阶段 1：日常体验

- 文件夹上传：目录树幂等预建、Worker 增量哈希、按目录并发上传队列
- 跨空间移动：配额迁移、标签重建、ACL 重置、外链撤销，整棵子树一致
- 高级筛选（类型 / 标签 / 创建人 / 时间）、标签管理与合并重挂
- 文件说明与自定义元数据、节点关注与变更摘要、评论编辑与 @ 提及（收件人按 ACL 过滤）
- 个人空间 / 待接管空间交接（合并到接收人空间或新建协作空间）、离职孤儿空间提醒超管

#### 阶段 3：分享与协作

- 文件收集链接：单文件上限 / 扩展名 / 是否要求姓名 / 累计上限策略，匿名提交以创建者身份落盘并自动重命名，提交记录与「收集到新文件」通知
- 外链增强：IP / CIDR 白名单、预览水印（外链页与站内）、二维码、短链（`bizType=drive_share`，撤销即停用）
- 访问申请：无权限用户申请角色，节点管理者审批 / 拒绝并可设到期时间，申请人可撤回；外链与临时授权到期前提醒
- 在线状态：详情抽屉心跳（Redis 汇聚）显示正在查看同一节点的用户

#### 阶段 4：治理与集成

- **法律保留**：对文件 / 文件夹（含子树）冻结删除、彻底删除、删版本与跨空间移动，回收站自动清理与版本修剪跳过保留项，空间交接随迁；
  详情抽屉与合规治理页设置 / 解除，变更通知节点管理者
- **空间归档**：部门 / 协作空间归档为只读（写操作 423），侧栏默认隐藏，共享空间页可找回并恢复
- **容量趋势与扩容审批**：空间治理列表附带近 30 天日增与预计用满天数；空间管理者申请扩容，网盘管理员审批通过即写入显式配额
- **外链访问日志治理**：全部访问留痕（含被拒绝尝试）按空间 / 外链 / 动作 / 结果 / 时间筛选，导出中心新增 `drive.share_access_logs`
- **异常行为告警**：每 5 分钟内置规则扫描短时批量下载（用户 / IP）与外链爆破（IP / 外链），阈值与窗口可配置，同一主体每窗口只告警一次
- **租户级设置**：`drive` 设置模块改为 `tenant` 作用域；后台任务、渲染 worker 与匿名外链入口按数据行 `tenantId` 显式读取
- **开放平台**：`/api/open/v1/drive/*` 开放 API（`drive:read` 空间 / 目录 / 元数据 / 内容，`drive:write` 上传），可见范围由「开放应用授权」按空间授予；
  `drive.node.*` / `drive.share.*` / `drive.collect.received` 文件变更事件经 pg-boss 延迟投递到 Webhook（事务确认后 emit，HMAC 必选，仅授权空间）
- **站内互通**：文件 / 文件夹以卡片消息「发送到聊天」（只带链接，按网盘 ACL 访问）；Wiki 编辑器「插入网盘文件」经 `DriveNodePicker` 写入站内链接
- 新页面「网盘管理 → 合规治理」（法律保留 / 扩容审批 / 外链访问日志 / 开放应用授权），菜单权限 `drive:admin:legal-hold:edit` /
  `drive:admin:quota:approve` / `drive:admin:open-grant:edit` / `drive:admin:link:export`，API Scope `drive:read` / `drive:write`
- 通知事件：`drive.quota.requested` / `drive.quota.decided` / `drive.security.alert` / `drive.legal_hold.changed`

### Changed

- 网盘工作台分栏去掉 `bordered`，与全站分栏页一致；协作空间新建 / 编辑改为 SideSheet，工作台「+」原地打开；空间治理页图表改为无卡片面板
- 应用统计标签样式改用 `.zx-flat-panels`
- 全工作区依赖升级 47 项（nodemailer 10、OTel 0.222、hono 4.13.7、pg-boss 12.30 等；`@mastra/evals` 锁定 `~1.9.0` 以兼容 vitest 5）
- 开放平台 Webhook：支付、退款与网盘事件必须使用 HMAC-SHA256 签名；平台域事件匹配支持按网盘空间授权过滤
- 聊天 `sendMessage` 增加服务端内部入参类型，允许系统生成 `card` 消息；`ForwardModal` 支持复用为通用会话选择器

### Fixed

- 短链目标安全校验误拦截本系统公开页（localhost / 内网 `publicBaseUrl`）
- 前端预览弹层（portal）下页面水印失效，改为独立预览覆盖层
- 收集链接「剩余可提交数」以服务端返回为准，避免前端双减
- 网盘服务中对 `timestamp` 列的裸 Date 参数比较（`sql\`col < ${date}\``）在 postgres-js 下无法序列化，统一改为类型化比较操作符

### 未交付项

- WebDAV 网关与 Electron 桌面同步客户端未实现：两者都需自实现协议 / 同步引擎并长期维护，与现有开放 API 相比投入产出不匹配，见 `docs/drive/index.md`
- 工作流没有专用的「网盘文件」表单字段类型，表单可用 `url` 字段引用网盘链接
...
- 空间治理：统计概览（空间 / 文件 / 占用 / 回收站与版本占用 / 有效外链 / 今日上传下载）、7 天趋势与文件类型占用图、空间列表治理（配额 / 状态 / 默认角色 / 所有者 / 外链开关）、创建部门空间、删除空间
