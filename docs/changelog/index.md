...
- **应用交付 / 部署管理**：应用版本页统一支持客户端应用与服务端应用；服务端版本使用 `server` / `archive` 制品，系统运维新增「应用部署」页，覆盖部署目标、异步部署记录、逐主机实时日志、滚动 / 并行发布、健康检查自动回滚、取消 / 重试、发布备份与 release 对账。
- 部署目录采用不可变 `releases/<timestamp-version>` + `current` 原子切换 + `shared` 持久化布局；制品上传复用现有分片与 sha256 校验，部署执行复用统一 SSH / SFTP 主机执行层与任务中心。
