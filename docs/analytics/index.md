...
| [行为分析](./behavior) | 概览 KPI、趋势、实时、页面停留、功能使用、会话、漏斗（有序转化 + 转化窗口 + 分群限定）、留存（真实首访/窗口首现双口径）、路径、用户时间线、维度分布、Web Vitals、点击分布、事件分析工作台（自定义维度/属性查询） |
| [数据管理](./data-management) | 事件明细与导出、事件字典治理、每日聚合、采集设置、数据保留策略、用户分群（CRUD + 异步物化）、报表中心数据集/看板复用 |
