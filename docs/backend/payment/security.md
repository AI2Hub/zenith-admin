...
| 回调验签 | 微信：按 `Wechatpay-Serial` 自动下载平台证书（12h 缓存，应对轮换）RSA-SHA256 验签 + AES-256-GCM 解密；支付宝：RSA2 验签 + 同步响应验签。处理幂等 + 原子，重放无害 |
...
| 外呼 | 全部走 [`http-client`](../http-client)（熔断 + Header 脱敏 + 结构化日志） |
| 权限 / 审计 | `payment:*` 权限码 + `guard({ audit })` 写操作日志；退款为高危操作需二次确认 |
...
- 微信平台证书按 `Wechatpay-Serial` 自动下载并缓存 12h，平滑应对证书轮换；
- 处理逻辑**幂等**：`markOrderPaid` / `finalizeRefund` 用原子条件更新（仅在订单 / 退款单尚未终态时更新），同一回调重放无副作用。

## 3. 幂等防重复

- **下单 / 退款**：路由挂 [`idempotencyGuard`](../idempotency)（15s 窗口，Redis `SET NX EX`，支持客户端 `X-Idempotency-Key` 或服务端自动指纹）；
- **回调履约**：靠业务侧原子条件更新 + 事件 `eventId` 去重，保证 exactly-once；
