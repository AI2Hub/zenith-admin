...
支付结果以**渠道异步回调**为主、**主动查单**为辅，并通过 **Outbox 事件表**保证履约事件「崩溃不丢」。三重机制共同保证订单状态最终一致、业务履约 exactly-once。
...
sequenceDiagram
    ...
    SVC->>BUS: 5.setImmediate 投递 'payment.succeeded'
    BUS->>BIZ: 履约 / 发货 / 开通会员
...
- 微信：按 `Wechatpay-Serial` 自动下载平台证书（12h 缓存，应对证书轮换）RSA-SHA256 验签 + `AES-256-GCM` 解密 `resource`；支付宝：RSA2 验签；
- 回调处理**幂等**：靠 `markOrderPaid` / `finalizeRefund` 的**原子条件更新**（仅在订单尚未终态时更新），重放无害。

## 2. Outbox 可靠投递

进程内事件总线在**进程崩溃**时可能丢失履约事件。为此引入 `payment_events` Outbox 表：
...
- **原子持久化**：支付 / 退款成功时，在更新订单状态的**同一事务**内 `recordEvent()` 写入 outbox 事件（`status='pending'`）；
...
| 任务 | 作用 |
| --- | --- |
| `closeExpiredOrders` | 关闭超 `expiredAt` 仍处于 `pending` / `paying` 的订单 |
| `paymentReconciliation` | 对 N 分钟内仍 `paying` 的订单主动查单（`queryPayment`），纠正状态（回调兜底） |
...
| 字段 | 说明 |
| --- | --- |
| `channel` / `scene` | 渠道 / 场景（`payment` 支付回调、`refund` 退款回调） |
...
后台「回调日志」页可按渠道 / 场景 / 验签结果 / 时间范围筛选，并查看原始 body 与请求头，便于排查验签失败、对账争议。详见 [后台管理页面](./admin)。
