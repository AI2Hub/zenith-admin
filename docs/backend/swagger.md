...
- **实体 DTO 中心仓库**：按业务域拆分至 `packages/server/src/lib/dtos/`（`iam` / `auth` / `dict` / `files` / `logs` / `notices` / `system` / `workflow` / `dashboard` / `region` / `messages`），`packages/server/src/lib/openapi-dtos.ts` 为 re-export barrel —— 所有路由统一从 `'../lib/openapi-dtos'` 导入，保证 Swagger Components 单一来源，**禁止在路由文件内本地重复声明 `.openapi('EntityName')` 的实体 DTO**
