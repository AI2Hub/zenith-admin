...
聊天核心表定义在 `packages/server/src/db/schema.ts`。
...
- 文件：`type = file`，`extra.asset.kind = file`，记录 `fileId` 以便服务端预览接口鉴权
...
聊天 WebSocket 同时承载音视频通话信令。`routes/ws.ts` 会处理 `rtc:*` 消息，优先按 `payload.to` 定向发送；没有 `to` 但包含 `conversationId` 时，转发给会话内其他成员。
