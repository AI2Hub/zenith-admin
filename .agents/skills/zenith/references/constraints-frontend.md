...
| 新增 / 编辑弹窗 | `hooks/useEditModal.ts` 编排 + `components/EditFormModal.tsx` 承接壳：`<EditFormModal modal={modal} width={660}>字段</EditFormModal>`（= `AppModal({...modalProps}) > Spin(detailLoading) > Form(key=formKey, formProps)`；`title` / `okText` / `okButtonProps` / `onCancel` 直接作属性覆盖，Form 之前的说明传 `header`，Form 额外属性传 `formProps`），抽屉形态 `<EditFormSheet modal={modal} width={720}>`（`SideSheet` + `ModalFooter(footerProps)`，缺省 `okText="保存"`，自定义页脚传 `footer`）。表单之外另有独立编辑区 / 双态页脚的复合弹窗保留裸 `AppModal` / `SideSheet` 并 `eslint-disable-next-line no-restricted-syntax -- 理由` | `useRef<FormApi>` + `editingRecord` + `try { validate() } catch` + `Toast` + 关闭四件套；页面里直写 `<AppModal {...modal.modalProps}><Spin …><Form key={modal.formKey} {...modal.formProps}>` 三层壳（ESLint 封禁，含 `SideSheet` 变体）；`wrapperClassName="modal-spin-wrapper"`（无 CSS 定义的死属性，ESLint 封禁） | 确定按钮永远转圈；异步详情进不了表单；下次「新增」带出上次记录；漏 Spin 时编辑先闪一次列表行占位值 |
...
| 列表页搜索状态 | 标准分页列表页用 `hooks/useListPage.ts` **契约模式** `useListPage({ contract: xxxContract, useList, defaults?, params?, enabled?, table? })`：筛选状态类型 = 契约 list query 去分页键，`listKey` 由契约派生，返回 `bind` / `bindKeyword` / `bindRange` / `handleSearch` / `handleReset` / `applySearch` / `filterQuery` / `listQuery` / `tableProps` / `toolbarProps` / `filterSchema`；搜索状态不是契约 query 形状时用映射模式（`defaults` + `listKey: xxxKeys.lists` + `toQuery`）；树形 / 不分页 / 客户端过滤 / 一页多列表 / 列表 hook 第二实参非 `enabled` 等非标准形态退一层用 `hooks/useListSearch.ts`。受控筛选控件一律整体绑定：`{...bind('status')}` / `{...bindKeyword('keyword')}`（额外接回车查询）/ 时间范围 `{...bindRange(['startTime', 'endTime'])}`；`Checkbox` 等非 `value` / `onChange` 形态的控件才取 `setField` | 页内 `useState` 双状态 + 手写 `invalidateQueries`；`interface SearchParams` + `defaultSearchParams` + `toQuery` 与契约 query 同形却逐页手写；把草稿 `draftParams` 当查询条件 | 点查询没反应 / 草稿直接触发请求 / 漏分页；筛选状态键与契约键漂移 |
| 列表 / 导出 / 深链的查询参数 | `useListPage` 的 `toQuery: (s) => ({ … })`（结果即返回的 `filterQuery`，给 `<ExportButton query={filterQuery} permission="…" />` / 深链共用）；非标准页用 `hooks/useFilterQuery.ts` 的 `const filterQuery = useFilterQuery({ … })`（`compactParams` 后按**内容**稳定引用，无依赖数组、映射里引用的外部值变化自然反映）。二者都丢弃 `undefined` / `null` / 空串、保留 `0` / `false`、保留键类型，由契约 `QueryOf` 类型检查；契约必填的作用域参数（`siteId` / `accountId`）走 `params`，不进映射；「勾选才筛选」的布尔开关写 `flag ? true : undefined`；时间区间直接展开 `...formatDateTimeRangeForApi(range)`（日期级选择器用 `formatDateRangeForApi` / `formatDateRangeValuesForApi`，写法见 [crud-frontend.md → 搜索工具栏筛选控件](./crud-frontend.md#搜索工具栏筛选控件)）。导出与列表确需不同条件（两套列表、导出附加维度）时另建对象并注释理由。弱类型 `compactQuery` 只用于拼查询串 / 透传非契约通道，纯函数 `compactParams` 只在 hook 之外（导出 / 深链工具函数）使用 | 页面内 `useMemo(() => compactParams({ … }), [submittedParams])` 手写依赖数组（ESLint 封禁）；列表调用里 `x \|\| undefined` 逐字段手写（ESLint 封禁 `submitted*.x \|\| undefined` 及其 `.trim()` 形态，以及任意 `useXxxList(...)` 列表 hook 实参与 `ExportButton query={...}` 内的 `x \|\| undefined`，不论草稿对象叫什么）、再为导出写一份 `buildExportQuery()`；`...(x ? { x } : {})` 条件展开；`const [ds, de] = range ?? []` + `ds ? formatDateForApi(ds) : undefined` 逐端点手写；导出取 `draftParams`（未提交的草稿） | 同一组字段两处维护漂移（导出多带 / 少带筛选）；各页对空串 / `0` / `false` 的取舍不一致；日期级区间误用秒级 helper 漏掉终止日 |
| 列表页工具栏排布 | `components/list-page` 的 `ListSearchToolbar`，标准页用**契约写法** `<ListSearchToolbar page={page} filters={['keyword', 'status', ['startTime', 'endTime']]} overrides={{ deptId: (p) => <DeptPicker {...p.bind('deptId')} /> }} extraFilters={…} create={…} actions={…} />`——控件由契约 query 的 `x-filter` 语义派生（keyword → `KeywordInput`「搜索 + 匹配字段」，enum → `StatusSelect` / `FilterSelect`（标签取契约声明的 `dict` / `options`），bool → 是 / 否，id → `NumberFilter`，成对时间端点 → `DateRangeFilter`），未声明语义的键必须走 `overrides` 否则报错；映射模式页用槽位写法（`keyword` / `filters` / `create` / `actions` + `onSearch` / `onReset`）。**边输边筛 + 刷新**、没有「查询」按钮语义的即时过滤页（进程 / 服务 / 容器 / 端口 / 拦截记录…）用同目录的 `InstantFilterToolbar` | 契约模式页面仍手写 `keyword={<KeywordInput placeholder=… />}` / `<StatusSelect items={statusItems} …>` 槽位；手写 `SearchToolbar` 的 `primary` / `mobilePrimary` / `mobileFilters` 双份 JSX、`renderKeywordSearch` 之类的渲染闭包；裸 `<Button icon={<RefreshCw />}>刷新</Button>` | 占位文案与服务端实际检索字段漂移；桌面 / 移动排布各页不一致；移动端漏掉查询或新增 |
...
| 标准操作列 | `components/list-page` 的 `useCrudOperationColumn({ permission: 'system:xxx', edit: modal, remove: deleteMutation, label: (r) => r.name, content?, hidden?, disabled?, extra? / extraBetween? / extraAfter?, width?, desktopInlineKeys? })`：`[...extra, 编辑, ...extraBetween, 删除, ...extraAfter]`，权限门控按 `:update` / `:delete` 派生（不按约定传 `permissions`，页面布尔门控传 `allow`），删除确认 + 执行 + 提示按 `deleteAction` 约定接好；只有删除或只有编辑、条件数组、`useMemo` 内定义的列继续 `createOperationColumn` | `createOperationColumn` 里手写 `{ key: 'edit', hidden: !hasPermission('…:update'), onClick: () => modal.openEdit(record) }` + `deleteAction({ hidden: !hasPermission('…:delete'), … })` 二元组 | 权限码后缀写错；删除确认标题不指明对象 |
| 列表删除动作 | `components/list-page` 的 `deleteAction`（操作列）/ `confirmAndDelete`（批量按钮、面板与右键菜单里的删除）；成功文案要带结果（条数等）时 `successMessage: (res) => …`，收尾 `onDeleted(res)`。ESLint 封禁 `confirmDelete({...})` 内再手写 `Toast.success`；确认后立即关弹窗不等待请求、需要 `icon` / `onCancel` 等 `confirmAndDelete` 未承载的 Modal 选项时保留 `confirmDelete` 并 `eslint-disable-next-line no-restricted-syntax -- 理由` | 操作列里手写 `{ key: 'delete', danger: true, onClick: () => confirmDelete({ onOk: async () => { await mutateAsync(); Toast.success('删除成功'); } }) }` | 漏 Toast / 漏 danger / 成功后忘清选中 |
| 批量启用 / 停用 | `components/list-page` 的 `batchStatusHandler({ selectedRowKeys, clearSelection, run, confirm?, disableLabel?, entity?, confirmContent?, danger?, successMessage? })`（空选中短路 → 按策略确认 → 执行 → 清选中 → Toast，破坏性停用走 `confirmDanger`）+ `toolbar-controls` 的 `BatchStatusButtons({ count, onChange, disableLabel?, danger?, loading? })`（或拆开的 `BatchEnableButton` / `BatchDisableButton`）；载荷形状（`{ ids, status }` / `{ ids, enabled }` / 两个 mutation）在 `run` 里适配 | 页内手写 `handleBatchStatus`（`Modal.confirm` + `mutateAsync` + 清选中 + `Toast.success`）与 `<Button theme="light" icon=…>批量启用 ({n})</Button>` 按钮对；域内再造 `useXxxBatchStatus` | 确认时机 / 文案 / 红色确认与是否清选中各域漂移 |
| 列表表格接线 | `useListPage` 返回的 `tableProps` 展开到 `ConfigurableTable`（`rowSelection` / `empty` / `rowKey` 经 `table` 选项传入）；非标准页用 `components/list-page` 的 `listTableProps(listQuery, { pagination: buildPagination })` | 手写 `dataSource` / `loading` / `onRefresh` / `refreshLoading` / `rowKey` / `size` / `bordered` 七件套；`pagination: (total) => ({ ...buildPagination(total), pageSizeOpts })` 手写覆盖（用 `pageSizeOpts` 选项） | 漏 `onRefresh` 没有刷新按钮；分页 total 取错 |
| 外部作用域切换回第 1 页 | `useListPage` / `useListSearch` / `usePagination` 的 `resetKey`（当前公众号 / 站点 / 空间 / 目录 / 统计天数…；多个来源传数组 `resetKey: [spaceId, folderId]`，按元素比较）：作用域变化时在同一渲染内回到第 1 页、筛选条件保留 | `useEffect(() => setPage(1), [scopeId])` 与 `useEffect(() => { resetPage(); }, [days])`（ESLint 已封禁两种形态）；在切换控件的 `onChange` 里手写 `setPage(1)` 属合法但不必要 | effect 版会先用旧页码请求一次新作用域的数据；各页漏写则切换后停在越界空页 |
| 列表多选 / 批量操作的选中状态 | `components/list-page` 的 `useRowSelection()`（在 `useListPage` / `useListSearch` 之前声明）：`selectedRowKeys` / `hasSelection` / `clear` / `rowSelection`；行键为字符串用 `useRowSelection<string>()`，行级禁用传 `{ extra: { getCheckboxProps } }`；`onSearch: clearSelection`、`onDeleted: clearSelection`、`useListPage({ table: { rowSelection } })`（非标准页 `listTableProps(q, { rowSelection })`）。表格行键与业务 id 类型不一致（CMS 页 `rowKey` 为字符串、状态存数字 id）或选中集合由父组件持有的页面保留自维护状态并 `eslint-disable-next-line no-restricted-syntax -- 理由`（ESLint 封禁 `rowSelection.onChange` 里的 `keys as …` / `(keys ?? []) as …` 断言） | `useState<number[]>([])` + `rowSelection: { selectedRowKeys, onChange: (keys) => set(keys as number[]) }` + 各处 `setSelectedRowKeys([])` | `keys as number[]` / `(keys ?? []) as` / `.map(Number)` 各页归一写法不一；查询 / 翻页 / 删除后是否清选中各页漂移 |
...
| 子列表 / Tab 内的分页状态 | `hooks/usePagination.ts` 的 `usePagination({ pageSize?, pageSizeOpts?, resetKey? })` → `page` / `pageSize` / `setPage` / `buildPagination`（含总数收缩后的页码钳制） | `useState(1)` + `useState(10)` + 手写 `pagination: (total) => ({ currentPage, pageSize, total, onPageChange, onPageSizeChange })` | 删除 / 筛选后停留在空页；页大小切换不回首页 |
...
| 支付域表单 / 筛选控件 | `pages/payment/payment-form-fields.tsx` 的 `PaymentAppField`（选中后由调用方重置依赖字段）/ `PaymentMerchantConfigField` / `PaymentCurrencyField` / `PaymentAppFilterSelect`；应用 + 绑定商户配置联动取数用 `payment-app-options.ts` 的 `useAppMerchantConfigLookup(selectedAppId)`；按应用给出可用支付方式用 `useAppPaymentMethodOptions(selectedApp, candidates)`（含权限门、渠道能力查询与退化策略）；提交前校验「应用 ↔ 商户配置」绑定用 `resolveAppConfigBinding(lookup, applicationId, channelConfigId)`；支付日志 / 事件的行内展开详情用 `payment-expanded-detail.tsx` 的 `PaymentExpandedDetail({ meta, sections })` | 逐页手写 `Form.Select field="applicationId"` / `channelConfigId` / `currency` 与 `[{ value: 'CNY', … }]`；逐页复制 ~35 行「启用方式 ∩ 渠道能力」memo 与 `paymentAppConfigId`；两页各写一份「ID 说明 + 小标题 + JsonBlock」展开块 | 必填文案、可搜索、宽度各页不一；能力退化策略两页漂移 |
| 报表资源页表单字段 | `pages/report/report-form-fields.tsx` 的 `ReportOwnerFolderFields({ userOptions, folderOptions, layout })`；批量启停走上表的 `batchStatusHandler` + `BatchStatusButtons` | 逐页手写 `ownerId` / `folderId` 两个 `Form.Select` | 必填 / 可搜索 / 布局各页不一 |
| 开放平台应用列 / 颜色 | `pages/open-platform/open-app-columns.tsx` 的 `openAppEnvironmentColumn()` / `openAppReviewStatusColumn({ title?, width? })` / `openAppScopesColumn({ title?, width?, color? })`，Tag 颜色 `OPEN_APP_ENVIRONMENT_TAG_COLOR` / `OPEN_APP_REVIEW_STATUS_TAG_COLOR`，筛选与表单选项取 shared 的 `OPEN_APP_ENVIRONMENT_OPTIONS` / `OPEN_APP_REVIEW_STATUS_OPTIONS` | 「我的应用」「OAuth2 应用」「调用统计」各写一份 `value === 'sandbox' ? 'orange' : 'blue'` 与 `OPEN_APP_ENVIRONMENTS.map(...)` | 同一实体的环境 / 审核配色三页漂移 |
| 侧栏 / 面板条目的「更多」菜单 | `components/NavListPanel` 的 `NavListItemActions({ items: [{ key, label, icon?, danger?, hidden?, disabled?, onClick }] })`（`NavListItem` 的 `extra` 槽或面板 `headerExtra`；全部隐藏时不渲染） | 逐页手写 `Dropdown trigger="click" … render={<Dropdown.Menu>…}` + `<Button theme="borderless" size="small" icon={<MoreHorizontal />} onClick={stopPropagation}>` + 图标 / 文案 `span` | 触发按钮忘阻止冒泡顺带选中条目；菜单项间距与危险色各页不一 |
...
| 消息模板编辑表单 / 发送日志页 | `pages/system/message-template-form.tsx` 的 `TemplateNameCodeRow` / `TemplateVariablesRemarkRows`；发送日志筛选对 `send-log-ui.tsx` 的 `SendLogStatusSourceFilters`（接 `bind()` 结果：`status={bind('filterStatus')} source={bind('filterSource')}`），共用列 `send-log-columns.tsx` 的 `sendLogSourceColumn` / `sendLogOperatorColumn` / `sendLogErrorColumn` / `sendLogStatusColumn` | 邮件 / 短信 / 站内信页各抄一份名称编码行、变量备注行、状态来源筛选与列 | 三页字段校验文案与列宽漂移 |
...
- 不经输入框直接筛选（点部门树 / 标签 / 收藏 / 保存的视图）用 `useListPage` / `useListSearch` 的 `applySearch(params)`；
...
- **搜索栏布局**：标准列表页统一用 `components/list-page` 的 `ListSearchToolbar`（`keyword` / `filters` / `create` / `actions`），
  ...
  一次取全量、在客户端边输边筛、没有「查询」按钮语义的即时过滤页（进程 / 服务 / 容器 / 端口…）用同目录的 `InstantFilterToolbar`
  （`primary` / `filters` / `onRefresh` / `onReset` / `actions` / `extra`：桌面 主区 → 筛选 → 刷新 → 重置 → 操作 → 说明，移动 主区 + 刷新，筛选进抽屉）。
  只有无查询语义的工具面板、多套筛选区或报表型筛选条（必选区间 / 维度切换、按钮带权限禁用态）才直接用
  `components/SearchToolbar.tsx` 的结构化模式（`primary` / `filters` / `actions` + `mobilePrimary` / `mobileFilters` / `mobileActions`），
  且 `mobilePrimary` / `mobileActions` 与桌面相同时**不要重复传**（缺省即回落到 `primary` / `actions`）。
...
- **表格样式**：统一 `<ConfigurableTable columns={columns} {...tableProps} />`（`tableProps` 来自 `useListPage`；
  非标准页用 `components/list-page` 的 `listTableProps(listQuery, { pagination: buildPagination })` 展开）——默认 `bordered` / `rowKey="id"` / `size="small"`，
  并接好 `dataSource` / `loading` / `onRefresh` / `refreshLoading`。
...
- **状态列**：启停开关列一律 `useStatusToggle(...).column()`（默认「状态」/ 80 / `fixed: 'right'`）；当前用户不可切换的只读启停状态列用
  `utils/table-columns` 的 `enabledStatusColumn()`（同位同宽，可传列属性覆盖），自定义列内再渲染时用 `renderEnabledStatusTag` / `DictTag`
  （文案与颜色跟随 `COMMON_STATUS_LABELS`；ESLint 封禁 `<Tag color={x === 'enabled' ? … : …}>` 手写两态标签，三态或文案语义不同的标签
  加 `eslint-disable-next-line no-restricted-syntax -- 理由`）；**禁止**逐页手写 `{ title: '状态', dataIndex: 'status', width: 80, fixed: 'right', render: renderEnabledStatusTag }`；
...
- **时间 / 日期列**：一律用 `utils/table-columns` 的 `dateTimeColumn(title, dataIndex, options?)`
  ...
  **纯日期范围**（如请假起止日期）保持单行，宽度取 `DATE_RANGE_COLUMN_WIDTH`（260）；
  其它时间区间 / 复合列宽度取 `DATE_TIME_COLUMN_WIDTH`，复合单元格内的时间值仍经 `DateTimeText` 渲染
