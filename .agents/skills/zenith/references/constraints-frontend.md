...
| 列表 / 导出 / 深链的查询参数 | `lib/query.ts` 的 `compactParams({ ... })`（丢弃 `undefined` / `null` / 空串、保留 `0` / `false`，键类型保留）：由 `submittedParams` **只映射一次**得到 `filterQuery`，再 `useXxxList({ page, pageSize, ...filterQuery })` 与 `<ExportButton query={filterQuery} permission="…" />` 共用；「勾选才筛选」的布尔开关写 `flag ? true : undefined`；时间区间直接展开 `...formatDateTimeRangeForApi(range)`（日期级选择器用 `formatDateRangeForApi` / `formatDateRangeValuesForApi`，写法见 [crud-frontend.md → 搜索工具栏筛选控件](./crud-frontend.md#搜索工具栏筛选控件)）。导出与列表确需不同条件（两套列表、导出附加维度）时另建对象并注释理由。弱类型 `compactQuery` 只用于拼查询串 / 透传非契约通道 | 列表调用里 `x \|\| undefined` 逐字段手写（ESLint 封禁 `submitted*.x \|\| undefined` 及其 `.trim()` 形态，以及任意 `useXxxList(...)` 列表 hook 实参与 `ExportButton query={...}` 内的 `x \|\| undefined`，不论草稿对象叫什么）、再为导出写一份 `buildExportQuery()`；`...(x ? { x } : {})` 条件展开；`const [ds, de] = range ?? []` + `ds ? formatDateForApi(ds) : undefined` 逐端点手写；导出取 `draftParams`（未提交的草稿） | 同一组字段两处维护漂移（导出多带 / 少带筛选）；各页对空串 / `0` / `false` 的取舍不一致；日期级区间误用秒级 helper 漏掉终止日 |
...
| 外部作用域切换回第 1 页 | `useListSearch` / `usePagination` 的 `resetKey`（当前公众号 / 站点 / 空间 / 目录 / 统计天数…；多个来源传数组 `resetKey: [spaceId, folderId]`，按元素比较）：作用域变化时在同一渲染内回到第 1 页、筛选条件保留 | `useEffect(() => setPage(1), [scopeId])` 与 `useEffect(() => { resetPage(); }, [days])`（ESLint 已封禁两种形态）；在切换控件的 `onChange` 里手写 `setPage(1)` 属合法但不必要 | effect 版会先用旧页码请求一次新作用域的数据；各页漏写则切换后停在越界空页 |
| 列表多选 / 批量操作的选中状态 | `components/list-page` 的 `useRowSelection()`（在 `useListSearch` 之前声明）：`selectedRowKeys` / `hasSelection` / `clear` / `rowSelection`；行键为字符串用 `useRowSelection<string>()`，行级禁用传 `{ extra: { getCheckboxProps } }`；`onSearch: clearSelection`、`onDeleted: clearSelection`、`listTableProps(q, { rowSelection })`。表格行键与业务 id 类型不一致（CMS 页 `rowKey` 为字符串、状态存数字 id）或选中集合由父组件持有的页面保留自维护状态并 `eslint-disable-next-line no-restricted-syntax -- 理由`（ESLint 封禁 `rowSelection.onChange` 里的 `keys as …` / `(keys ?? []) as …` 断言） | `useState<number[]>([])` + `rowSelection: { selectedRowKeys, onChange: (keys) => set(keys as number[]) }` + 各处 `setSelectedRowKeys([])` | `keys as number[]` / `(keys ?? []) as` / `.map(Number)` 各页归一写法不一；查询 / 翻页 / 删除后是否清选中各页漂移 |
