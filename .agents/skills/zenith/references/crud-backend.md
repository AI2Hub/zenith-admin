...
本文档提供后端各层的代码模板，对照 `packages/server/src/routes/identity/users.ts` 和 `packages/server/src/db/schema/core.ts` 的实际实现。
...
## Step 5：Service 层（`packages/server/src/services/{业务域}/xxx.service.ts`）
...
## Step 6：OpenAPIHono Router（`packages/server/src/routes/{业务域}/xxx.ts`）
...
**薄路由约定**：**禁止在路由 handler 中直接调用 `db.*`**。所有 DB 访问与业务逻辑必须放在 `services/{业务域}/xxx.service.ts`；路由只负责：取参数 → 调 service → 返回 `c.json(okBody(...))`或透传错误（由全局 `onError` 将 `HTTPException` 转为标准 JSON）。DB 唯一约束 `23505` 也统一在 service 中通过 `rethrowPgUniqueViolation(err, msg)` 映射。
...
| 中间件 | `packages/server/src/middleware/guard.ts` | 自动从响应体提取 `afterData`；提供 `setAuditBeforeData()` / `setAuditAfterData()` 供路由注入快照 |
| 路由 | 需要 diff 的 PUT/DELETE/分配类路由 | 写操作前调用 `setAuditBeforeData(c, before)`；响应 `data` 为 null 但仍需 diff 时，写操作后调用 `setAuditAfterData(c, after)` |
...
1. `guard` 中间件自动：
   - 优先使用 `setAuditAfterData(c, after)` 注入的操作后快照
   - 未手动注入时，在 `next()` 后从 `{ code: 0, data: ... }` 响应体提取 `afterData`
...
成员分配、角色/菜单权限分配、数据权限设置等接口通常返回 `okBody(null, '保存成功')`。这类接口如果只设置 `beforeData`，操作日志会有流水但 diff 缺少变更后快照；必须在写操作后查询新状态并调用 `setAuditAfterData`。

```ts
const assignRoute = defineOpenAPIRoute({
  route: createRoute({
    method: 'put',
    path: '/{id}/members',
    middleware: [authMiddleware, guard({ permission: 'system:xxx:assign', audit: { description: '分配成员', module: 'XXX管理' } })] as const,
    // ...
  }),
  handler: async (c) => {
    const { id } = c.req.valid('param');
    ...
    const before = await getXxxMembersBeforeAudit(id);
    if (before) setAuditBeforeData(c, before);

    await setXxxMembers(id, userIds);

    const after = await getXxxMembersBeforeAudit(id);
...
> **命名建议**：分配/成员维护类 helper 可命名为 `getXxxMembersBeforeAudit()` / `getXxxPermissionBeforeAudit()`；返回主实体标识字段 + 关联 id 数组 + 轻量展示字段，避免把敏感字段或超大对象写入日志。
...
import {
  ...
  removeBusinessFile,
  listBusinessFiles,
} from '../../services/files/business-files.service';
```
...
- 公告模块：`packages/server/src/routes/messaging/announcements.ts`
- 附件服务：`packages/server/src/services/files/business-files.service.ts`
